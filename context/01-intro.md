...
- **[03-requirements.md](./03-requirements.md)** — Functional and
  governance requirements, out-of-scope items, and design
  characteristics that satisfy them.
- **[04-use-cases.md](./04-use-cases.md)** — Device classes and
  use case scenarios (pre-roll, mid-roll, coexisting overlay,
  hybrid linear + overlay, pause-triggered ad, multi-ad break,
  legacy Player encountering new constructs). Each scenario is
...
- **[07-backward-compat-checklist.md](./07-backward-compat-checklist.md)** —
  ...
  construct it introduces, so legacy Player compatibility is
  auditable per construct.
- **[08-dash-extension-rules.md](./08-dash-extension-rules.md)** —
  DASH 6th edition rules that close the design space for SGAI
  extensions. Consumed by the requirements that govern admissible
