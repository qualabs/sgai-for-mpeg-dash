...
- [x] **Spec — UC-09 listen-mode safety cap policy**: RESOLVED —
  ADR-01 in `.project/decisions/`. Decision: Broadcaster MAY declare
  a safety fallback (not SHOULD, not SHOULD NOT).
- [ ] **Spec — UC-09 non-linear listen-mode construct**: follow-up PR
  gated on the syntax chapter defining the non-linear event construct
  (schema, namespace, scheme URI). UC-09 in context/ is scoped to
  linear only until then. No TBD sits inside context/.
