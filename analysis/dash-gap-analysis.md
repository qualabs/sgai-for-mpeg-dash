...
6th edition (ISO/IEC 23009-1:2025) and identifies the gaps the spec
must close.
...
Player behaviour. This lives in chapter 7 of the spec (Expected
...
linear slots. For non-linear (overlay) slots, the spec has to
...
non-linear ad slots the spec needs a different resolution document
...
The spec has to decide between (a) extending `ListMPD` to make Period
...
The spec has to declare a normative convention: at minimum, require
...
quartile semantics is new and the spec has to spell it out alongside
...
spec has to resolve it (the conservative default per R2 is "decline
...
devices in this scenario"; the spec should keep this default and
flag it for the WG.

## 4. Reuse opportunities (R9)

DASH 6th's machinery covers more of SGAI than naive reading suggests.
The spec SHOULD reuse rather than introduce, in these places:
...
The following items need WG input or further research before the spec
publishes a final draft:
...
8. **VAST 4.x version pin** [inferred]: the exact 4.x version the
   spec cites in its informative annex (4.0 / 4.1 / 4.2 / 4.3) needs
