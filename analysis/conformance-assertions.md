...
| R1.2 | context/03-requirements.md:36 | spec document | (unconditional) | Every new SGAI construct introduced by the proposal MUST be expressed using a MPEG-DASH 6th edition extension point whose ignore-if-unknown semantics are already defined. |
| R1.3 | context/03-requirements.md:40 | spec document | (unconditional) | The spec MUST NOT alter or override the semantics of any pre-existing MPEG-DASH 6th edition construct. |
...
| R2.4 | context/03-requirements.md:60 | spec document | Given any new mechanism introduced by the spec | MUST be expressible within the three-actor contract; mechanisms outside the contract MUST be rejected or redesigned. |
| R3.1 | context/03-requirements.md:74 | spec document | (unconditional) | The spec MUST enumerate the supported device classes and, for each class, the expected behaviour for each ad opportunity type covered by the use cases. |
| R3.2 | context/03-requirements.md:77 | Player | Given any ad opportunity type defined in the spec | MUST produce a defined behaviour (render, fall back, or skip); undefined behaviour is non-conforming. |
...
| R6.1 | context/03-requirements.md:184 | spec document | (unconditional) | MUST specify how in-band ad tracking beacons are carried in the resolution document. |
...
| R6.3 | context/03-requirements.md:190 | spec document | Given a new tracking carrier is proposed | MAY be introduced only when the callback scheme cannot express the required semantics, and only after a documented gap analysis per R9. |
...
| R8.1 | context/03-requirements.md:307 | spec document | Given a new construct introduced by the proposal | MUST be accompanied by an inline justification stating why an existing MPEG-DASH construct could not be reused. |
| R8.2 | context/03-requirements.md:311 | spec document | Given a deliberate omission of an existing MPEG-DASH construct | MUST be documented inline with the design decision. |
| R9.1 | context/03-requirements.md:321 | spec document | (unconditional) | MUST reuse existing MPEG-DASH machinery (events, manifests, presentations, schemes) wherever possible. |
| R9.2 | context/03-requirements.md:324 | spec document | Given a candidate new construct | MUST NOT introduce it unless an existing one cannot be made to fit. |
| R9.3 | context/03-requirements.md:327 | spec document | Before introducing a new construct | MUST consider whether an extension to an existing construct would suffice, and document the outcome. |
| R10.1 | context/03-requirements.md:338 | spec document | (unconditional) | Spatial arrangement of overlays MUST be delegated to HTML5 / CSS layout primitives. |
| R10.2 | context/03-requirements.md:340 | spec document | (unconditional) | The spec MUST NOT define a parallel layout standard for overlay placement. |
| R10.3 | context/03-requirements.md:342 | spec document | Given position semantics inside a layout | MUST be expressed via Positioning Templates using HTML5 / CSS primitives. |
| R11.1 | context/03-requirements.md:247 | spec document | (unconditional) | Normative chapters MUST NOT cite a specific VAST version as required. |
| R11.2 | context/03-requirements.md:249 | Player | Given an ADS that does not use VAST | MUST be able to operate against it. |
| R11.3 | context/03-requirements.md:252 | spec document | Given a reference to VAST in the spec | MUST appear in an annex or non-normative note explicitly flagged as illustrative. |
| R12.1 | context/03-requirements.md:266 | spec document | (unconditional) | The list of accepted ad-type values MUST be sourced from IAB definitions, with a reference to the IAB source. |
...
| R12.3 | context/03-requirements.md:273 | ADS | Given a candidate's form metadata | MUST NOT emit form metadata for ad types not part of the IAB-defined set used by this spec's edition. |
...
| R13.4 | context/03-requirements.md:294 | spec document | (unconditional) | MUST NOT introduce a new tracking event scheme; reuse of the linear baseline tracking mechanism is mandatory. |
...
| N06.1 | context/06-naming-and-namespaces.md:22 | spec document | Given a new event scheme introduced by the spec | MUST use the year-pinned pattern `urn:svta:dash:<construct>:<year>`. |
| N06.2 | context/06-naming-and-namespaces.md:43 | spec document | Given tracking carrier semantics | MUST inherit the existing DASH 6th edition callback scheme; introducing a parallel tracking scheme under `urn:svta:dash:*` is out of scope. |
...
| C07.1 | context/07-backward-compat-checklist.md:17 | spec document | Given any new construct C introduced by the spec | MUST explicitly answer the 7-item per-construct checklist (placement, extension rule, walk-through, sibling check, UC-07 test, namespace, doc cross-reference). |
| C07.2 | context/07-backward-compat-checklist.md:74 | spec document | Given any new construct C | MUST ship a UC-07-modelled test case in chapter 10 (representative MPD, legacy Player harness, expected silent-skip behaviour). |
| G99.1 | context/99-glossary.md:23 | spec document | Given the Overlay umbrella term | MUST reference IAB-defined sub-types normatively and MUST NOT introduce new sub-types. |
...
- spec document: 19 (R1.2, R1.3, R2.4, R3.1, R6.1, R6.3, R8.1, R8.2, R9.1, R9.2, R9.3, R10.1, R10.2, R10.3, R11.1, R11.3, R12.1, R13.4, N06.1, N06.2, C07.1, C07.2, G99.1, UC04.2)
- all (cross-cutting): 0
