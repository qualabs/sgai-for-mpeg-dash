...
> Build metadata
> - Iteration: v5.1 (minor refinement of v5)
...
<!-- refine: v5-spec-validation.md#G-1 -->
- Parallel non-linear ad rendering. At most one non-linear ad form
  is active on the screen at any given moment. The serialisation
  obligation lives on the Player at runtime (§4.4.7) and is
  composed with the pause-ad-over-overlay precedence rules in
  §4.4.8 and §7.6; no manifest-level attribute parameterises this
  constraint in the current edition.
...
- The pause-ad form is rendered on top of the paused primary
  frame.
<!-- refine: v5-detail-review.md#flag-4 -->
- When an overlay opportunity (§5.1.3) is also active at the same
  instant, the Player renders the pause-ad form on top and
  suspends the overlay rendering. The detailed timeline-anchor
  behaviour of the overlay's slot-window clock during the pause is
  specified in §7.6.
...
<!-- refine: v5-spec-validation.md#EC-3 -->
A Player encountering an `<svta:OverlayPresentation>` or
`<svta:PauseAdPresentation>` event at `presentationTime=0` on the
first `<Period>` treats the construct as an authoring error and
applies the standard fall-through (§4.4.1, E5-equivalent): the
event is ignored and primary content continues uninterrupted. No
resolution request is issued and no tracking beacon is fired.
...
<!-- refine: v5-detail-review.md#flag-2 -->
NOTE: Unlike `<svta:OverlayPresentation>` (§5.1.3),
`<svta:PauseAdPresentation>` does not declare
`@earliestResolutionTimeOffsetTicks`. The Earliest Resolution Time
(ERT) for a pause-trigger slot is the moment the viewer pauses
inside the window of validity (§6.2); there is no pre-pause
resolution window to parameterise. The omission is deliberate, not
an oversight.
...
<!-- refine: v5-spec-validation.md#EC-4 -->
A conforming ADS authors the wrapping `<Period>@duration="PT0S"`
verbatim. A Player encountering an Overlay Resolution Document
whose `<Period>@duration` is non-zero (and therefore lacks the
`<AdaptationSet>` child required by §5.3.2.2 Table 4 of the base
specification) treats the document as schema-invalid and applies
E2 (§8.1): the document is discarded and the slot falls through.
...
Both attributes are named `@duration` and typed `xs:duration` in
seconds. The name reuse is deliberate: per the naming-consistency
rule in `context/06-naming-and-namespaces.md` N06.13, when an
SGAI-side attribute carries semantics matching a baseline DASH
attribute, the SGAI attribute reuses the baseline name verbatim
...
<!-- refine: v5-spec-validation.md#A-1 -->
When a non-linear `video` form carries `<EventStream>` callbacks
both inside its SPS sub-MPD (§5.5.2 row 1) AND as a child of
`<svta:RenderableAsset>` (§5.5.3), the inline
`<svta:RenderableAsset>`-attached `<EventStream>` is the canonical
beacon-set for that rendering instance: its events fire and the
sub-MPD events are suppressed for the same rendering. The two
carriers MUST NOT both fire — the spec's beacon-set per
rendering instance is single-rooted. The preferred placement
(per §5.5.4) is the inline `<svta:RenderableAsset>` carrier;
sub-MPD callbacks remain useful for linear ad tracking and for
video-form tracking that aligns to the ad's intrinsic duration
rather than the slot window.
...
The following items from the v5 analyses could not be resolved
...
|---|--------|----------|---------|--------------------------------------|
| 1 | validation | G-2 | Form tie-break preference order `[inferred]` (video > html > image) on §7.3 step 5 (3). | Requires a normative tie-break vocabulary in `context/03-requirements.md` R5. Spec-text cannot invent the order without a context anchor. |
| 2 | validation | G-3 | Click-tracking beacon lifecycle after slot trim (§4.4.3 / §4.4.9). | `context/` is silent on whether a viewer click after trim fires the tracking beacon. Resolution requires a new R13.x clause; either outcome is requirement-level. |
| 3 | validation | EC-1 | Authoring case where the overlay window is longer than the linear window in a hybrid slot. | §5.1.5 already encodes the clamp-to-linear-end behaviour; the missing piece is the explicit UC-04 walkthrough in `context/04-use-cases.md`. Context-level. |
| 4 | validation | EC-2 | `<svta:Click>@through` URL malformed or unreachable. | Host-environment dependent; requires a normative Player obligation in `context/03-requirements.md` R6.x or a Player conformance statement convention. |
| 5 | validation | A-2 | `<svta:Candidate>@priority` redundancy with declared candidate order (DP-1.2 single source of truth). | Requires `context/03-requirements.md` R7 to state whether `@priority` is diagnostic (drop the attribute) or a Player-side tie-break (keep, with explicit semantics). |
| 6 | validation | A-3 | Pause-ad speculative pre-fetch staleness threshold. | §8.5 is non-normative; a normative staleness boundary would be a new requirement. Player-side policy in this edition. |
| 7 | detail-review | flag-1 | `@earliestResolutionTimeOffsetTicks` suffix-naming policy. | Requires `context/06-naming-and-namespaces.md` to introduce a normative suffix convention for unit-disambiguated attribute names. Naming-policy decision. |
| 8 | detail-review | flag-5 / audit M-1 | `<EventStream>` under `<svta:RenderableAsset>` with reinterpreted `presentationTime` reference frame. | Requires `context/08-dash-extension-rules.md` to introduce a new DR-8 admitting "baseline DASH element under SVTA-namespaced parent with reinterpreted timing", or to rename the SGAI-side tracking carrier. Architectural decision. |
