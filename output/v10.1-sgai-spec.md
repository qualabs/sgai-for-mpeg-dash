...
**Document status.** Candidate specification, build iteration 10.1. It
...
ADS and the APS are external services no one audits. The Player never <!-- refine: v10-spec-validation.md#T1 -->
talks to the ADS: the resolution document is the only thing it reads
about an ad, and a Player MUST operate whatever decision format the ADS
uses. In deployed systems the APS is often a module of the ADS;
...
- **Creative carriers other than video, image and HTML.** Raw
  ...
  excluded. A scripted creative MUST be wrapped in an HTML document and <!-- refine: v10-spec-validation.md#T1 -->
...
| Overlay window | First option in order: video, image or HTML overlay, or a squeezeback | Video overlay; a `squeezeback-double-box` (without background) or an L-shape with a video creative; otherwise skip <!-- refine: v10-detail-review.md#flag-2 --> | Image or HTML overlay; squeezeback with an image or HTML creative | Image overlay; squeezeback with an image creative | Skip, unless the window admits `linear` and the candidate offers it |
...
§4.5 addressed to that actor. Each obligation is stated positively, <!-- refine: v10-spec-validation.md#T1 -->
with its RFC 2119 keyword; what lies outside an obligation is outside
the contract.
...
1. **MUST declare every constraint on a slot in the main MPD** — its cap, <!-- refine: v10-spec-validation.md#T1 -->
   its family, its allowed layouts, its custom region, its
   early-resolution offset, its once-per-session bound — and MUST leave none
   of them to be inferred at runtime by the ADS, the APS or the Player.
2. **MUST declare `@maxDuration` on every ad slot of every family**, linear <!-- refine: v10-spec-validation.md#T1 -->
...
5. **MUST author every window of one family in a Period inside a single <!-- refine: v10-spec-validation.md#T1 -->
   `<EventStream>`** of that family's scheme (§5.1.5), with no `@value`
   on an SGAI scheme's stream.
6. **MUST draw allowed layouts only from the tokens of §3.4.2** that are <!-- refine: v10-spec-validation.md#T1 -->
...
10. **MUST request the play-list metric** with a `<Metrics>` element on the <!-- refine: v10-spec-validation.md#T1 -->
...
1. **MUST decide which ads to serve for an opportunity, how many and in <!-- refine: v10-spec-validation.md#T1 -->
...
2. **MUST emit its decision in its own decision document**, typically VAST, <!-- refine: v10-spec-validation.md#T1 -->
...
<!-- refine: v10-spec-validation.md#T1 -->
The ADS is not required to respect the slot cap when selecting
candidates, and a conformance check on an ADS MUST NOT fail on the
...
2. **MUST convert the ADS's decision, in whatever format the two agreed, <!-- refine: v10-spec-validation.md#T1 -->
...
4. **MUST express an opportunity that resolved to no ads as an empty <!-- refine: v10-spec-validation.md#T1 -->
...
5. **Carries each candidate's presentation options as an ordered list
   ...
   is the APS's decision. <!-- refine: v10-spec-validation.md#T2 -->
6. **MUST emit only admissible layouts**: tokens of §3.4.2, admissible for <!-- refine: v10-spec-validation.md#T1 -->
   the document's family (§5.3.3), and within the set the request
   forwarded (`sgaiAllowedLayouts`) or, when it forwarded none, within
   the family default (§3.4.3). A `custom` option MUST carry a <!-- refine: v10-spec-validation.md#T1 -->
   `@customRectangle` lying inside the region the request forwarded, or
   inside the viewport when it forwarded none.
7. **MUST emit only admissible creatives**: every option's form is one of <!-- refine: v10-spec-validation.md#T1 -->
   ...
   addressed by `@assetUrl` and MUST NOT be addressed by a `@mimeType`
   on an Adaptation Set or a Representation.
8. **MUST carry a background image on every `squeezeback-double-box-background` <!-- refine: v10-spec-validation.md#T1 -->
   option** in `@backgroundUrl`, as a composition attribute of the
   layout (§5.3.6).
9. **MUST translate the ADS's tracking schedule into callback events** <!-- refine: v10-spec-validation.md#T1 -->
   (§5.5), with times relative to each ad's presentation, and MUST carry a
   ClickThrough, when the ad has one, in `<svta:Click>` together with any
   click-tracking URLs (§5.6).
10. **MUST declare, on every non-linear resolution document, whether the <!-- refine: v10-spec-validation.md#T1 -->
    slot is dismissible** by carrying `@dismissAfter` when it is and
    omitting it when it is not (§5.2.4).
11. **MUST declare `@usableFor` whenever its decision stays good for less <!-- refine: v10-spec-validation.md#T1 -->
    than the window lasts** (§5.2.5); omitting it declares that the
    document is usable for the whole window.
12. **MUST declare `@onCandidatesExhausted` on every pause document** <!-- refine: v10-spec-validation.md#T1 -->
    (§5.2.6).
13. **Answers without any capability parameter.** The APS MUST produce <!-- refine: v10-spec-validation.md#T1 -->
...
2. **The Player MUST NOT resolve an overlay window earlier than its <!-- refine: v10-spec-validation.md#T1 -->
   earliest resolution time** — the window's `@presentationTime` minus its
   `@earliestResolutionTimeOffset`, declared or 60 seconds by default —
   and at the latest while the window is still open.
3. **The Player MUST NOT resolve a pause window earlier than its <!-- refine: v10-spec-validation.md#T1 -->
   earliest resolution time**, computed against the window's start and
   never against the pause. When a viewer pause begins inside a pause
   window, the Player MUST hold a usable resolution for it: one obtained earlier and still
...
5. **When an overlay or pause opportunity fires, the Player MUST check <!-- refine: v10-spec-validation.md#T1 -->
   that the resolution it holds is still usable.** The Player MUST NOT
   present a resolution past its `@usableFor`: it MUST request a new one,
   and a new one that yields no usable candidate is an empty resolution
   (§4.5.6) — the expired resolution MUST NOT be used as a fallback.
6. **The request MUST carry the forwarded declarations** of §5.8.3 <!-- refine: v10-spec-validation.md#T1 -->
   ...
   disclose (§5.8.4). Any other parameter it adds MUST carry a vendor
   prefix.
7. **The Player MUST present no ad in a window whose `@maxDuration` is <!-- refine: v10-spec-validation.md#T1 -->
   absent**, and MUST continue the primary content. The Player MUST NOT
   read the absence as the base specification's unbounded default. A window whose `@maxDuration` is
   zero does not fire.

#### 4.5.2 Validating what it receives

1. **The Player MUST validate every candidate against the declarations <!-- refine: v10-spec-validation.md#T1 -->
   of the window that served it** before rendering anything, and MUST
   render only candidates that satisfy them.
2. **A document is a resolution of the requested slot only if its
   ...
   overlay or pause window. Any other document is a failed execution of <!-- refine: v10-spec-validation.md#T1 -->
   that window (§4.5.6), and the Player MUST present none of its
   candidates.
...
1. **For each candidate, the Player MUST walk the presentation options <!-- refine: v10-spec-validation.md#T1 -->
   in document order** and render the **first** one that satisfies both:
...
2. **The Player MUST NOT render an option that fails either check**, and <!-- refine: v10-spec-validation.md#T1 -->
   MUST move to the next option in document order. The check holds whether
...
3. **The Player MUST skip a candidate with no option passing both <!-- refine: v10-spec-validation.md#T1 -->
   checks**, and MUST move to the next candidate. Only when every
   candidate is exhausted does it continue with the primary content. A skipped
   candidate is not a failed execution of the window.
4. **The Player MUST NOT attempt to render a form its device cannot <!-- refine: v10-spec-validation.md#T1 -->
   render.** A Player that does not support `custom` MUST treat every
   `custom` option as not renderable.
...
|---|---|---|
| Linear, replacement | **Until when** | <!-- refine: v10-spec-validation.md#T1 --> The Player MUST end the presentation no later than the end the base `@clip` semantics fix: with `@clip="true"` (the default) the scheduled end, so a late start shortens it; with `@clip="false"`, one cap-length after the actual start. |
| Linear, insertion | **How long**: the cumulative duration of what the slot presents | The Player MUST stop rendering once that duration would exceed the cap, even mid-ad. <!-- refine: v10-spec-validation.md#T1 --> |
...
| Pause | **Nothing** | The viewer sets the length of a pause slot. The cap is declared and MUST NOT be interpreted as bounding a duration. <!-- refine: v10-spec-validation.md#T1 --> |

1. **The Player MUST NOT extend a slot beyond what the cap bounds for <!-- refine: v10-spec-validation.md#T1 -->
   its family**, whatever the ADS metadata or the number of candidates say.
2. **The Player MUST enforce the cap against actual rendered length**, <!-- refine: v10-spec-validation.md#T1 -->
   not declared length: an accepted ad whose actual length exceeds its declared
...
4. **Timebases are reconciled before comparing.** The cap is in units of
   ...
   The Player MUST convert the duration into the cap's timescale and round <!-- refine: v10-spec-validation.md#T1 -->
   **up** to the next whole unit — each ad's duration separately, before
   the cumulative sum is taken; it MUST admit a converted duration equal
   to the cap.
5. **The Player MUST measure the cap on the presentation timeline.** An <!-- refine: v10-spec-validation.md#T1 -->
   interval in which the presentation timeline does not advance MUST
   accrue nothing: a
...
1. **The Player MUST present the candidates of a resolution document in <!-- refine: v10-spec-validation.md#T1 -->
   the order the document declares**, dropping only a candidate with no
   renderable option (§4.5.3) or one dropped before play under §4.5.4.
2. **The survivors keep their order.** The Player MUST NOT reorder, <!-- refine: v10-spec-validation.md#T1 -->
...
1. **When windows of one family overlap in time, the Player MUST order <!-- refine: v10-spec-validation.md#T1 -->
   them by presentation time, oldest first, and take windows with equal
   presentation times in the order they appear in the stream.** It MUST
   attempt the first.
2. **An attempt that produces no ad is a failed execution, and on a <!-- refine: v10-spec-validation.md#T1 -->
   failed execution the Player MUST attempt the next overlapping window
   of the family.** This is the base specification's rule for the linear
...
5. **When every overlapping window has been attempted and none produced <!-- refine: v10-spec-validation.md#T1 -->
   an ad, the Player MUST continue with the primary content
   uninterrupted** — *"If no event can be successfully executed, the
   playback continues uninterrupted"* (DASH §5.16.2.2.5).
6. **A failed execution consumes nothing.** The Player MUST NOT count an <!-- refine: v10-spec-validation.md#T1 -->
   empty resolution as an execution: a linear event with `@executeOnce="true"`
...
<!-- refine: v10-spec-validation.md#T1 -->
The Player MUST validate the candidates each window of a fallback chain
serves against **that window's own** declarations — its allowed layouts,
its custom region and its cap — and MUST NOT validate them against those
of the window it stands in for. A declaration belongs to the window that carries it.

#### 4.5.8 One non-linear form at a time, in sequence

1. **At any instant the Player MUST keep at most one non-linear ad form <!-- refine: v10-spec-validation.md#T1 -->
   ...
   two.
2. **When a non-linear document carries more than one candidate, the <!-- refine: v10-spec-validation.md#T1 -->
   Player MUST present them in sequence**, in document order, each starting
...
1. **While the viewer is paused inside a pause window and an overlay is <!-- refine: v10-spec-validation.md#T1 -->
   active, the Player MUST render the pause ad and suspend the overlay**,
   whether the pause ad is fullscreen or partial. The pause ad is the
   only ad surface visible.
2. **On resume, the Player MUST dismiss the pause ad and restore the <!-- refine: v10-spec-validation.md#T1 -->
...
3. **When a viewer pause begins inside a pause window while a linear ad <!-- refine: v10-spec-validation.md#T1 -->
   occupies the screen, the Player MUST present the pause ad and suspend
   the linear ad**, and on resume MUST continue the linear ad from where
   it was suspended.
...
1. **A pause ad exists only while the primary content is paused.** On
   the pause-to-play transition the Player MUST remove any rendered pause <!-- refine: v10-spec-validation.md#T1 -->
   ad within one rendering frame and MUST return to the primary content
   immediately, whether or not an ad is mid-presentation.
2. **From that transition on, the Player MUST NOT fire any further <!-- refine: v10-spec-validation.md#T1 -->
   beacon of the dismissed pause ad**; beacons scheduled after it fall outside the
...
3. **The Player MAY present a pause ad fullscreen or as a partial
   ...
   by any pre-existing overlay. For a partial one, the paused frame stays <!-- refine: v10-spec-validation.md#T1 -->
   visible and the Player MUST keep any coexisting overlay suspended
   (§4.5.9).
4. **A pause is what the viewer experiences, not how the Player achieves
   ...
   resources for the pause. On resume it MUST continue the primary <!-- refine: v10-spec-validation.md#T1 -->
   content from that position; a mechanism that cannot restore the position is
...
5. **In live content, while the viewer is paused inside a pause window,
   the Player MUST keep its presentation time frozen inside that window** for <!-- refine: v10-spec-validation.md#T1 -->
   ...
   window (§8.8).
6. **On a window with `@executeOnce="true"`, the Player MUST present at <!-- refine: v10-spec-validation.md#T1 -->
   most one pause ad for that window for the whole session**, and a later
   qualifying pause inside it leaves the primary content untouched. The
   Player MUST consume the window when a pause ad **begins rendering**,
   not when the pause occurs: a pause that resolves to no renderable candidate leaves
   the window available.
7. **The Player MUST NOT interpret the cap as bounding a pause slot** <!-- refine: v10-spec-validation.md#T1 -->
   (§4.5.4).
...
none. Under `request-again`, the Player MUST treat a document carrying no <!-- refine: v10-spec-validation.md#T1 -->
candidates, or none the device can render, as `stop` for the rest of
that pause, so that one
...
1. **The Player MUST render every ad form, linear or non-linear, at the <!-- refine: v10-spec-validation.md#T1 -->
   playback speed of the primary content** at the moment the ad is
   presented, and MUST NOT force an ad to 1× when the primary content
   plays at another speed.
2. **Durations stay on the presentation timeline.** The on-screen
   wall-clock length of a form MUST be derived as `duration / <!-- refine: v10-spec-validation.md#T1 -->
   playback_speed`; a 10-second ad presented at 2× is on screen for 5
   seconds. Cap enforcement and beacon scheduling MUST use the
...
3. **This holds for every form**, including `image` and `html`, which
   have no intrinsic media: their declared duration is a <!-- refine: v10-spec-validation.md#T1 -->
   presentation-timeline value and their wall-clock length MUST be
   derived the same way.

#### 4.5.13 Tracking

1. **For every ad accepted for rendering, the Player MUST execute the <!-- refine: v10-spec-validation.md#T1 -->
...
2. **When the cap trims an ad, the Player MUST stop firing that ad's <!-- refine: v10-spec-validation.md#T1 -->
   remaining beacons at the trim boundary.** A beacon scheduled exactly
   at the boundary fires: the ad reached that time.
3. **The Player MUST fire each beacon once within its ad** (§5.5.4), and <!-- refine: v10-spec-validation.md#T1 -->
   MUST fire both of two beacons that share an `@id` in two different ads.
4. **The Player MUST resolve a callback stream inside a <!-- refine: v10-spec-validation.md#T1 -->
   `<svta:Candidate>` against that candidate's own presentation.**
5. **The Player MUST ignore extension elements and attributes of a <!-- refine: v10-spec-validation.md#T1 -->
   namespace it does not implement**, on tracking-related constructs as anywhere
   else, and keeps playing.

#### 4.5.14 Viewer dismissal

1. **On a non-linear slot, the Player MUST make dismissal available only <!-- refine: v10-spec-validation.md#T1 -->
   as `@dismissAfter` declares** (§5.2.4): not at all when the attribute is
...
2. **On a linear slot, the Player honours the base declarations with
   ...
   declaration allows.
3. **A dismissal ends the whole slot.** The Player MUST stop presenting <!-- refine: v10-spec-validation.md#T1 -->
   every ad of the slot and MUST NOT advance to any other ad or form
   within it.
4. **A dismissed slot does not shorten the primary content.** Where the <!-- refine: v10-spec-validation.md#T1 -->
   slot bounded a region of the primary timeline, the Player MUST
   continue from where the primary content stands and MUST NOT skip any
   of it.
5. **The Player MUST fire the beacons scheduled up to the moment of the <!-- refine: v10-spec-validation.md#T1 -->
...
<!-- refine: v10-spec-validation.md#T1 -->
A Player conformant to this specification **MUST read `<svta:Click>`**
and, when the viewer activates the ClickThrough, MUST open (or hand to
the platform) `@clickThroughUrl` and fire every `<svta:ClickTracking>`
URL once per activation. Click-tracking fires on activation and never on the timeline.
...
1. **When resolving or rendering an accepted ad fails at runtime** — a
   ...
   fetched, a network loss mid-ad — **the Player MUST abort that ad and <!-- refine: v10-spec-validation.md#T1 -->
   continue the primary content uninterrupted.** This is the base
   ...
   here.
2. **On every device class and for every opportunity type, the Player <!-- refine: v10-spec-validation.md#T1 -->
   MUST produce a defined behaviour**: it renders, falls back to a later
...
`PlayList` entries as §5.9 describes, and MUST NOT count a playback
...
2. **Extension schema.** Every element of the namespace
   ...
   §5.10, and the one base-namespace element admitted inside a <!-- refine: v10-dash-conformance-audit.md#C24 -->
   `<svta:Candidate>` validates strictly as the base `EventStreamType`.
   The §5.10 wildcard admits that element laxly, because the base schema
   gives `EventStream` no global declaration, so the validator binds it
   to `EventStreamType` explicitly; schema validation of the document
...
4. **Conditional rules.** The rules the schema cannot express hold: the
   ...
   documents only; every non-linear option carries `@duration` and <!-- refine: v10-spec-validation.md#T3 -->
   `@layout`, and an option on a List MPD Period carries no `@layout`.
...
- **Walk-through:** a Player that does not implement this specification
  ...
  and processes the rest of the attribute. <!-- refine: v10-dash-conformance-audit.md#C34 --> The mechanism the URN rides
  on is not free of legacy cost: a Publisher using `<RequestParam>`
  carries the MPD-level `EssentialProperty` of scheme
  `urn:mpeg:dash:urlparam:2025` that DASH §I.3.1 requires, and a client
  that does not recognise that scheme ignores the whole MPD (DASH
  §5.8.4.8). That cost is the base specification's, not a construct of
  this one.
...
| `@maxDuration` | **yes under this specification**; optional in the base schema | `xs:unsignedLong` | none under this specification (the base default is infinity, carried in the base schema as `default="2251799813685247"`) <!-- refine: v10-dash-conformance-audit.md#C13 --> | The slot cap, in units of the parent `EventStream@timescale`. On an insertion slot it bounds the cumulative duration of what the slot presents (§4.5.4). Zero means the event is not executed. |
...
requires a value on every slot (§4.8.3). <!-- refine: v10-dash-conformance-audit.md#C13 --> Absence is judged on the
document as authored, before any schema default is applied: the base
schema declares `default="2251799813685247"` on the attribute, and a
value supplied by that default is not a declaration, so a Player of this
specification reads whether the attribute is lexically present rather
than the value a defaulting parser hands it. The zero rule is inherited as
...
| `<svta:RenderableAsset>` | no | 0..N | Further presentation options for the same ad, each with `@form` `image` or `html` (§5.3). <!-- refine: v10-spec-validation.md#T3 --> They follow the `<ImportedMPD>` in document order, so the video is the first option. |
...
<!-- refine: v10-spec-validation.md#T1 -->
An opportunity that resolved to **no ads** MUST be answered with HTTP
`200` and a resolution document that is well-formed and complete and
carries no candidate. It MUST NOT be an empty body, a `204`, a `404`, or
...
| `@layout` | yes on a non-linear candidate; absent on a List MPD Period, whose family fixes it to `linear` | layout token | — | <!-- refine: v10-spec-validation.md#T3 --> The layout, drawn from §3.4.2. The Player matches it against the window's allowed layouts by exact token. Value space in §5.3.3. |
...
|---|---|
| A List MPD Period (linear) | none: the attribute is absent, and the option is `linear` <!-- refine: v10-spec-validation.md#T3 --> |
...
`squeezeback-l-shape-upper-right` — MUST carry exactly **one** ad <!-- refine: v10-spec-validation.md#T1 -->
creative, an image, a video or an HTML document, and that creative is
always **full-frame, in the background**. The Player MUST shrink the
primary content into the region the token names and composite it **on
top of** the creative. The "L" is the band of the creative that stays visible
...
content and the ad in two boxes side by side, and the Player MUST
composite them as those two boxes. The two boxes leave bands uncovered,
and without a background those bands render **black**.

**The double box with background** —
`squeezeback-double-box-background` — adds a third element: the <!-- refine: v10-spec-validation.md#T1 -->
advertiser's **background image** at `@backgroundUrl`, which the Player
MUST place in the uncovered bands. The background is always a still image, never video
...
that makes an option satisfiable or not: the Player MUST evaluate it <!-- refine: v10-spec-validation.md#T1 -->
...
presentation is that Period's timeline. For a stream inside a <!-- refine: v10-dash-conformance-audit.md#C25 -->
`<svta:Candidate>`, it is the presentation of the option the Player
selected for that candidate: the start of that option replaces the
Period start that anchors `@presentationTime` in the base definition
(DASH Table 44), and does not extend it. `EventStream@presentationTimeOffset`
keeps its base role on such a stream — it is subtracted from each
`@presentationTime` — so a stream that omits it, and takes its default
of zero, writes its times from the option's start. An `image` or `html` option has no
...
presentation time, MUST fire once — including across the two positions <!-- refine: v10-spec-validation.md#T1 -->
of §5.5.2. Across scopes it does not reach: the same `@id` in two
candidates of one document is two beacons, and the Player MUST fire
both.
...
<!-- refine: v10-spec-validation.md#T1 -->
When a non-linear window declares `@allowedLayouts`, the Player MUST send
...
declares `@customRegion`, the Player MUST send that too.
...
- Sending a capability parameter is **OPTIONAL**. A Player MAY send all
  three, some, or none.
- A parameter the Player has no value for, or does not disclose, MUST <!-- refine: v10-spec-validation.md#T1 -->
  be **omitted entirely** — never sent empty or with a placeholder.
...
- **Every parameter name beginning with `sgai` is reserved** to this and
  later editions. A parameter a Player attaches that is not a reserved <!-- refine: v10-spec-validation.md#T1 -->
  name MUST carry a vendor prefix of the form `x-<vendor>-`, so that names
...
<!-- refine: v10-spec-validation.md#T1 -->
The Publisher of content carrying pause windows MUST request the metric
...
namespace `urn:svta:dash:sgai:2026`. It imports the base MPD schema. <!-- refine: v10-dash-conformance-audit.md#C24 -->
The base schema declares `EventStream` only as a local element, so the
wildcard that admits the callback stream inside `<svta:Candidate>` is
lax: a strict wildcard would demand a global declaration the base schema
does not provide, and reject every document carrying that stream. The
schema therefore does not check that element; §4.6 step 2 does. Where a table in this chapter states a conditional
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xs:element name="Candidate">
    <xs:complexType>
      <xs:sequence>
        ...
        <!-- refine: v10-dash-conformance-audit.md#C24 -->
  ...
  <xs:element name="RenderableAsset">
    <xs:complexType>
      ...
      <xs:attribute name="layout" type="svta:LayoutTokenType"/> <!-- refine: v10-spec-validation.md#T3 -->
...
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011"
  ...
  <Period id="ad-7731" duration="PT15S">
    ...
    <!-- refine: v10-spec-validation.md#T3 -->
...
- **The candidates follow the ADS's order**, and the options inside <!-- refine: v10-spec-validation.md#T2 -->
  each candidate are in preference order (§5.3.4).
...
<!-- refine: v10-detail-review.md#flag-1 -->
...
MPD root, the Period, and the candidate's `<EventStream>`,
`<svta:Click>` and metadata are as above, elided here.
...
<svta:OverlayList xmlns:svta="urn:svta:dash:sgai:2026" family="overlay">
  <svta:Candidate id="c1">
    <svta:RenderableAsset form="video"
                          ...
                          backgroundUrl="https://ads.example.com/c1/background.jpg"/>
    <!-- refine: v10-detail-review.md#flag-1 -->
    <!-- callback EventStream, svta:Click and metadata as in the D4 document -->
  </svta:Candidate>
</svta:OverlayList>
```
...
| S-4 | §4.4 item 5 | A resolution document with a multi-option candidate | Options in preference order (§5.3.4) | An option list the Player cannot read as ordered: reported <!-- refine: v10-spec-validation.md#T2 --> |
...
The following items from the v10 analyses could not be resolved
...
|---|--------|----------|---------|--------------------------------------|
| 1 | validation | 5.b-B1 (EC-2) | When a once-only linear or overlay window answered with only unrenderable candidates is consumed | Flagged for human review (§5.b): writing the rule is authoring a new sentence |
| 2 | validation | 5.b-B2 (A-28) | §6.6 read as illustrative or as the normative R11.4 coverage statement | Flagged for human review (§5.b): two defensible fixes |
| 3 | validation | 5.b-B3 (A-29) | "Uninterrupted" defined by a list of prohibitions in §4.5.16 item 3 | Flagged for human review (§5.b): the positive restatement has more than one form |
| 4 | validation | 5.b-B4 (A-30) | Test S-10 checks that a ClickThrough arrived | Flagged for human review (§5.b) |
| 5 | validation | 5.c-C1 (EC-1) | A blackout replacement without `@maxDuration` presents no replacement | Deferred to `context/` (R4.1 / R4.10) |
| 6 | validation | 5.c-C2 (A-12) | Which linear skip declaration counts under R35.8 | Deferred to `context/` (R35.8, 06); §4.5.14 item 2 left as it stands |
| 7 | validation | 5.c-C3 (A-11) | R34.2 "at most one pause ad for that window" | Deferred to `context/` |
| 8 | validation | 5.c-C4 (A-15) | R36.6 "no usable candidate" | Deferred to `context/` |
| 9 | validation | 5.c-C5 (A-19) | UC-07: whether a current Player plays the standard break | Deferred to `context/` |
| 10 | validation | 5.c-C6 (A-1) | R1 / R1.1 bind a Player that does not implement the specification | Deferred to `context/`; §1.4 and §4.1 left without a modal |
| 11 | validation | 5.c-C7 (A-3) | DP-1.2 against the pre-fetch duration of a video option | Deferred to `context/` |
| 12 | validation | 5.c-C8 (G-1) | Request-level extension hooks not admitted by R1.2 / DR-1..DR-10 | Deferred to `context/` |
| 13 | validation | 5.c-C9 (A-5) | R11 "VAST-version-agnostic" unverifiable for an APS | Deferred to `context/` |
| 14 | validation | 5.c-C10 (A-8) | R15.2 "forms declared by the Publisher" | Deferred to `context/` |
| 15 | validation | 5.c-C11 (A-20) | UC-08 reuse-signal clause | Deferred to `context/` |
| 16 | validation | 5.c-C12 (A-24) | UC-16 overlay form unstated; D2 cell | Deferred to `context/` |
| 17 | validation | 5.c-C13 (A-31) | Nine indicative obligations in `context/` with no modal | Deferred to `context/`; their spec sites keep the indicative |
| 18 | validation | 5.c-C14 (A-4) | R1.3 against R4.10 / R1.5 | Deferred to `context/` |
| 19 | validation | 5.c-C15 (A-2) | R2.2 against R38.4 | Deferred to `context/` |
| 20 | validation | 5.c-C16 (A-6) | R31.1 against R36.5 | Deferred to `context/` |
| 21 | validation | 5.c-C17 (A-7) | R4.9 rounding per ad or on the sum | Deferred to `context/` |
| 22 | validation | 5.c-C18 (A-9) | R12.4 against R39 | Deferred to `context/` |
| 23 | validation | 5.c-C19 (A-10) | R7.2 against R5.3 | Deferred to `context/` |
| 24 | validation | 5.c-C20 (A-13) | D5 and the pause ad against R37 | Deferred to `context/` |
| 25 | validation | 5.c-C21 (A-14) | R36.4 lifetime declaration | Deferred to `context/` |
| 26 | validation | 5.c-C22 (A-16) | R14.2 against R31.2 | Deferred to `context/` |
| 27 | validation | 5.c-C23 (A-17) | R26.1 slot or layout, and the Publisher's role | Deferred to `context/` |
| 28 | validation | 5.c-C24 (A-18) | Universal ad identifier carrier | Deferred to `context/` |
| 29 | validation | 5.c-C25 (A-21) | Side-by-side on D2 in UC-03 / UC-10 | Deferred to `context/` |
| 30 | validation | 5.c-C26 (A-22, A-23) | UC-04 L-shape over a takeover; UC-12 against R20.1 | Deferred to `context/` |
| 31 | validation | 5.c-C27 (EC-3..EC-8) | Actual vs declared length, per-ad base skip, held resolution expiry, speed changes, per-option schedules, overlay closed at a live resume | Deferred to `context/`: each is silent there |
