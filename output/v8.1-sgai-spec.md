...
- **Ad types outside the closed set of §3.2**, including every ad
  ...
  <!-- refine: v8-spec-validation.md#T1 (OOS-4#p1) -->
- **Creative carriers outside video, still image and HTML** (§3.3).
  A scripted creative MUST be wrapped in an HTML document rather than
...
<!-- refine: v8-dash-conformance-audit.md#M1 -->
<!-- refine: v8-spec-validation.md#T5 -->
...
| IETF RFC 8141, *Uniform Resource Names (URNs)* | April 2017 | The production this specification elects for the URNs it mints. DASH §8.1 requires it of the base specification's own profile identifiers and leaves an externally defined one free — *"Externally defined profiles may use profile identifiers that are URNs or URLs"* — so this is an election and not an obligation. |
...
**Informative references.** IAB Tech Lab, Video Ad Serving Template
(VAST) 4.x — illustrative only. Named in §6.7 as an example of what an
Ad Decision Server typically emits. No construct, obligation or
behaviour in this document depends on VAST or on any VAST version.
...
<!-- refine: v8-detail-review.md#flag-1 -->
Each URI above is `urn:svta:dash:<construct>:<year>`, where
`<construct>` is a multi-segment path whose first segment names the
category — `event`, `profile`, `request` — mirroring the base
specification's own `urn:mpeg:dash:event:callback:2015` and
`urn:mpeg:dash:profile:list:2024`. The year suffix is the edition year.
A construct whose semantics change in a later edition takes a new year;
one whose semantics are unchanged keeps its URI. A Player implementing
a later edition SHOULD recognise both that edition's URIs and this
edition's.
...
<!-- refine: v8-spec-validation.md#T1 (R2.1) -->
...
resolution-timing bound, any once-per-session bound — MUST be declared
in the MPD by the Publisher. No actor infers them at runtime.

<!-- refine: v8-spec-validation.md#T1 (R4.1) -->
**4.3.2 Declare a maximum duration on every slot.** Every ad slot the
MPD declares, linear or non-linear, MUST carry `@maxDuration`.
...
<!-- refine: v8-spec-validation.md#T1 (R12.2) -->
**4.3.3 Draw allowed-layout names from the closed set.** Each token in
a slot's `@allowedLayouts` MUST be one of the tokens of §3.2, each of
which maps one-to-one to an IAB-defined ad type or visual placement. For a
pause-trigger window the Publisher names a surface — `pause-fullscreen`,
`pause-partial`, or both.

<!-- refine: v8-spec-validation.md#T1 (R20.2) -->
**4.3.4 Author one event stream per family per Period.** All the
opportunity windows of one family that share a `<Period>` MUST be
authored as `<Event>` entries inside a **single** `<EventStream>`. The base
...
<!-- refine: v8-spec-validation.md#T1 (R33.4) -->
**4.3.8 Declare the pause metric where pause windows exist.** Content
carrying pause-trigger windows MUST request the `PlayList` metric
through the base specification's `Metrics` element (§5.9). Collection is
...
<!-- refine: v8-spec-validation.md#T1 (R1.1, R1#p2) -->
**4.3.9 Author the document so that removal leaves it valid.** The
Publisher MUST author the manifest so that a client which does not
implement this specification removes every construct introduced here
and what remains is a valid, conforming document. This is the base
specification's own authoring
obligation: *"the MPD shall be authored such that, after XML attributes or
...
conforms to this document"* (DASH §5.2.1). A Publisher who wants a Player
that predates this specification to see something in place of a non-linear
opportunity authors that something out of baseline constructs, as a sibling
rather than nested inside an extension element (§4.7.4).
...
<!-- refine: v8-spec-validation.md#T1 (R2.2) -->
**4.4.1 The ADS decides, and emits a decision document.** It MUST
select which ads serve an opportunity, how many and in what order, and
MUST emit them in its own format. It performs no conversion into a
DASH-native shape.
...
<!-- refine: v8-spec-validation.md#T1 (R11#p2, R11.2) -->
The APS is the only non-Player actor whose output this specification
checks. Every obligation below MUST be checked against the resolution
document alone, which is the one artefact on the path to the Player
that this specification defines: a Player conformant to this
specification MUST operate from that document whatever the ADS emits,
and never addresses the ADS itself (§6.7).

<!-- refine: v8-spec-validation.md#T1 (R2.2) -->
**4.5.1 Answer the resolution request with a resolution document.**
The APS MUST return HTTP `200` with a body that parses and validates: a
...
<!-- refine: v8-spec-validation.md#T1 (R5.1) -->
**4.5.3 Carry the presentation options as an ordered list.** Each
candidate MUST carry one or more presentation options as an ordered
list whose document order is the preference order. How many a candidate
...
<!-- refine: v8-spec-validation.md#T1 (R12.3, R15.2) -->
**4.5.4 Keep the ad-type vocabulary inside the closed set.** Every
presentation option's `@layout` MUST be a token of §3.2, and every
creative MUST carry a media type inside the admissible set of §3.3.

<!-- refine: v8-spec-validation.md#T1 (R24#p2, R24.1) -->
**4.5.5 Carry non-audiovisual creatives on a conformant carrier.** The URL
of an `image` or `html` creative MUST travel on the carrier of §5.3.2.
Video creatives travel through `<ImportedMPD>` into a sub-MPD bound by the
base specification to the Single-Period Static profile.

<!-- refine: v8-spec-validation.md#T1 (R13.1) -->
**4.5.6 Carry the tracking schedule the ADS declared, on the callback
scheme.** Beacons MUST be `<Event>` entries inside an `<EventStream>`
of scheme `urn:mpeg:dash:event:callback:2015`, with presentation times
on the ad's own presentation timeline (§5.5). The APS transcribes the
...
<!-- refine: v8-spec-validation.md#T1 (R30.1) -->
**4.5.7 Express an unsold opportunity as a document.** An opportunity
that resolved with no ads MUST be answered with a well-formed,
complete resolution document carrying no candidates (§5.2.3), served
with `200` and a body.

<!-- refine: v8-spec-validation.md#T1 (R28.1) -->
...
accompanying it MUST travel together in the carrier of §5.6. Whether a
...
<!-- refine: v8-spec-validation.md#T1 (R29.5) -->
**4.5.9 Answer without any capability parameter.** The APS MUST produce
...
answer would make the Player's freedom to omit them unattainable. An absent
parameter means its value is **undetermined**, never that the device lacks
the capability.

<!-- refine: v8-spec-validation.md#T1 (R32.1) -->
**4.5.10 Declare the pause-exhaustion behaviour.** A resolution
document for a pause slot MUST declare which of `repeat`,
`request-again` and `stop` applies when the candidates run out while
the viewer is still paused (§5.2.2.3). Absent the declaration the
Player MUST apply `stop`.

**4.5.11 Reference media that stays reachable.** The ad media a
candidate points at is reachable for the duration of the slot.

<!-- refine: v8-spec-validation.md#T4 -->
**4.5.12 A device-capability view is optional.** Producing candidates
requires no device-class matrix and no per-Player capability view at
the APS either. An APS that holds one is equally conformant, and an APS
that holds none answers every request (§4.5.9).
...
<!-- refine: v8-spec-validation.md#T1 (R1.4) -->
**4.6.1 Keep the primary content playing.** Through every condition in
this chapter and in §8.1 the Player MUST keep primary-content playback
...
error, a malformed candidate, a mid-ad network loss — the Player MUST
abort that ad and MUST continue the primary content.

<!-- refine: v8-spec-validation.md#T1 (R31.1) -->
...
pause-trigger window the Player MUST issue the request when a viewer
pause begins inside the window, and MUST NOT issue one for a pause that
begins outside every such window.

<!-- refine: v8-spec-validation.md#T1 (R2.3, R5#p3) -->
**4.6.3 Validate every candidate against the window that served it.**
Before rendering anything from a candidate, the Player MUST check each
presentation option against (a) what its device can satisfy and (b) the
`@allowedLayouts` of the window that served the candidate, and MUST
render only an option that satisfies both.

**4.6.4 Enforce the slot cap.** What the cap bounds differs by family,
because the base specification makes it so:

<!-- refine: v8-spec-validation.md#T1 (R4.3, R4.6) -->
<!-- refine: v8-spec-validation.md#T1 (R31.2) -->
| Family and operation | What `@maxDuration` bounds |
|---|---|
| Linear replacement | **Until when.** With `@clip` at its default `true` the alternative presentation *"shall terminate at the latest at time PRT + APDmax"* (DASH Table 62) — the end the Publisher scheduled, whatever time the event actually fired, so a late start MUST shorten the ad rather than move the end. With `@clip="false"` it terminates at `PRTA + APDmax` and the slot ends later than scheduled. |
...
| Pause | **Nothing.** A pause slot's duration is set by the viewer (§4.6.10), and the cap MUST NOT be read as bounding the slot. `@maxDuration` on a pause-trigger window bounds the display duration of a single pause ad before automatic dismissal. |
...
<!-- refine: v8-spec-validation.md#T1 (R4#p1, R4.2, R4.5, R7.5) -->
Where the cap bounds cumulative duration the Player MUST stop
rendering once the cumulative duration would exceed it, **even when the
stop falls mid-ad**, and MUST enforce against **actual rendered
length** rather than declared length. The Player MAY additionally drop
a candidate before playback when its declared duration alone would push
the cumulative duration past the cap. In short: the Player MAY drop
before play on declared duration, and MUST trim during play on actual
length.

<!-- refine: v8-spec-validation.md#T1 (R4.9) -->
...
duration as an ISO 8601 `xs:duration`. The Player MUST convert the
candidate's duration into the cap's timescale before comparing, and MUST
round the converted value **up** to the next whole unit. A candidate whose
...
<!-- refine: v8-spec-validation.md#T1 (R4.10) -->
A slot declaration carrying no `@maxDuration` is not a slot this
specification defines (§4.3.2). The Player MUST present no ad from such a
slot and MUST continue with the primary content. Reading the absence as the
base specification's unbounded default would make cap enforcement inert for
exactly the slots whose declaration is defective.
...
<!-- refine: v8-spec-validation.md#T1 (R5#p1, R5.2, R5.6, R3.3) -->
**4.6.5 Walk the presentation options in document order.** For an
accepted candidate the Player MUST evaluate the options in document
order and MUST render the **first** whose form and layout it can
satisfy on its device and whose layout the window admits. An option
that fails either check MUST be passed over and the Player MUST move to
the next option in document order. The Player MUST render no form its
device cannot render.

<!-- refine: v8-spec-validation.md#T1 (R5#p2, R5.3, R5.7) -->
**4.6.6 Fall through candidate by candidate.** A candidate with no
satisfiable option MUST be skipped and the Player MUST advance to the
next candidate in the resolution document. The Player MUST continue
with the primary content only once every candidate is exhausted. A
...
<!-- refine: v8-spec-validation.md#T1 (R7#p1, R7.1, R7#p2, R7.4) -->
**4.6.7 Honour the document's order, and present one form at a time.**
The Player MUST present the candidates in the order the resolution
document declares. It MAY drop a candidate that has no satisfiable
option or whose declared duration would overrun the cap; the candidates
that survive MUST be presented in the order the document declared them,
without reordering, deduplication or rearrangement.

<!-- refine: v8-spec-validation.md#T1 (R22.1) -->
At any instant the Player MUST keep at most **one** non-linear form active
on the screen. Sequenced forms — one ending, the next beginning — are in
scope; two forms on screen at the same instant are not. The bound exists so
that the device never needs more than the primary content plus one ad
form's decoder concurrently: two concurrent video overlays would demand
three decoders, and many target devices support two.

<!-- refine: v8-spec-validation.md#T1 (R14.1, R14.2) -->
Within a non-linear slot, candidates MUST be presented one after another in
declared order, each starting when the previous ends, and the cap MUST be
enforced against the cumulative duration of that sequence. Each candidate
contributes the single presentation option the Player selected for it, so
the sequence is a sequence of candidates and the alternatives inside a
candidate are not part of it.

<!-- refine: v8-spec-validation.md#T1 (R20.1) -->
**4.6.8 Treat an attempt that produced no ad as a failed execution.**
When windows of one family overlap in time, the Player MUST select the
first (§4.6.9) and attempt to resolve it. An attempt **on a window**
that produces no ad is a **failed execution**, and on a failed
execution the Player MUST attempt the next overlapping window of the
same family.
...
Player MUST treat all four alike:
...
<!-- refine: v8-spec-validation.md#T1 (R30.2) -->
An empty resolution MUST NOT **consume** the opportunity. The base
...
<!-- refine: v8-spec-validation.md#T1 (R20.4) -->
A resolution document whose **family does not match** the slot that
requested it is a failure to resolve, and the Player MUST continue down
the chain. It cannot fill this slot whatever it contains, because its
...
<!-- refine: v8-spec-validation.md#T1 (R16.1, R16.2) -->
A pause ad exists only while playback is paused. On the pause-to-play
transition the Player MUST remove any rendered pause-ad form from the
screen within one rendering frame and MUST stop firing beacons scheduled
for it; beacons scheduled at relative times after the transition fall
outside the pause ad's active window.
...
<!-- refine: v8-spec-validation.md#T1 (R32.1, R32.2) -->
Absent a declaration the Player MUST apply `stop`, which is what the
...
candidates MUST be treated as `stop` for the remainder of that pause. The
candidate-level fall-through of §4.6.6 does not apply here, because the
primary content is paused.

<!-- refine: v8-spec-validation.md#T1 (R32.3) -->
<!-- refine: v8-spec-validation.md#T3 -->
When the viewer resumes, the Player MUST return to the primary content
immediately, whether or not an ad is mid-presentation. Whether a second
resolution request inside one pause is the same opportunity or a new one
is out of scope. The two readings are identical at the Player — it
requests, it renders what arrives, and it stops on resume, in both — and
differ only in accounting between the APS and the ADS, which this
specification does not observe (§1.2).

<!-- refine: v8-spec-validation.md#T1 (R34.2) -->
**4.6.12 Honour a once-per-session pause window.** On a pause-trigger
window declared `@executeOnce="true"` the Player MUST present at most
one pause ad for that window for the duration of the session, and a
later qualifying pause inside the same window MUST leave the primary
content uninterrupted. The window is consumed when a pause ad **begins
...
<!-- refine: v8-spec-validation.md#T1 (R19#p1, R19.1, R19.2, R19.3, R19.4) -->
**4.6.14 Render at the primary content's speed.** Every ad form,
linear or non-linear, MUST be rendered at the speed the primary content
is playing at the moment the ad is presented; the Player MUST NOT force
an ad to 1× while the primary content runs at another speed. A form's
...
Player MUST derive the wall-clock on-screen length as
...
<!-- refine: v8-spec-validation.md#T1 (R13.2, R13.3) -->
**4.6.15 Execute the tracking schedule the document carries.** For an
accepted candidate the Player MUST fire each beacon at its scheduled
relative time, preserving the ADS's authority over the schedule. Where
the cap trims the ad before a scheduled beacon's time, the Player MUST
stop firing the remaining beacons at the trim boundary.

<!-- refine: v8-spec-validation.md#T1 (R6.5) -->
...
distinct beacons and the Player MUST fire both. Where the beacon carrier
...
<!-- refine: v8-spec-validation.md#T1 (R28.2) -->
**4.6.16 Fire the click and its tracking on activation.** A Player
conformant to this specification MUST read the ClickThrough URL from
the carrier of §5.6 and, when the viewer activates the ClickThrough,
MUST open or hand off the destination and MUST fire each accompanying
click-tracking URL once.

<!-- refine: v8-spec-validation.md#T1 (R25#p1, R25.1) -->
<!-- refine: v8-spec-validation.md#T1 (R25.1) -->
**4.6.17 Freeze presentation time for a pause ad in live content.** In live
content, while the viewer is paused inside a pause-trigger window, the
Player MUST keep its presentation time frozen inside that window for the
duration of the pause, even though the live edge keeps advancing in
wall-clock time: the window is anchored to the Player's frozen presentation
time rather than to the advancing live timeline, so the pause ad stays
admissible until the viewer resumes. A decision to resume at the live edge
MUST be treated as a Player action occurring **after** the resume from
...
<!-- refine: v8-spec-validation.md#T1 (R33.2) -->
**4.6.18 Derive the paused interval from the base specification's
metric.** A Player that reports metrics MUST derive the paused interval
from the `PlayList` entries as §5.9 describes, and MUST count no
playback period that stopped on `Rebuffering` as a pause opportunity.

<!-- refine: v8-spec-validation.md#T1 (R26.2, R27.2) -->
**4.6.19 Compose the layouts as declared.** For an L-shape the Player
MUST composite the two elements — the full-frame ad creative in the
...
MUST composite the shrunk primary content and the ad as the two boxes;
where the advertiser supplied a background element it MUST place it in
the uncovered bands, and where none was supplied the uncovered region
renders as black.
...
<!-- refine: v8-spec-validation.md#T1 (R6#p2, R6.4) -->
...
MUST be ignored together with its whole subtree, and the primary
content MUST continue uninterrupted.

<!-- refine: v8-spec-validation.md#T1 (R29.3, R29.4) -->
...
have, or does not disclose, MUST be **omitted entirely** rather than
sent empty; a parameter the Player attaches that is not one of the
reserved names MUST carry a vendor-specific prefix, so that reserved
...
<!-- refine: v8-spec-validation.md#T1 (R24#p1) -->
A non-MP4 payload MUST NOT be wrapped in an `application/mp4`
Representation to satisfy the registry: the wrapper adds no
segment-delivery semantics for the underlying format,
...
<!-- refine: v8-dash-conformance-audit.md#M1 -->
DASH §8.1 also recommends the term for an externally defined
identifier: *"It is recommended that such external definitions be not
referred to as profiles, but as Interoperability Points."* The value
`urn:svta:dash:profile:sgai-overlay-list:2026` carries the token
`profile` and §5.2.2 calls it a profile URI. Renaming a published
identifier is a breaking change and the base specification's
recommendation is not an obligation, so the existing URI is kept and
...
<!-- refine: v8-spec-validation.md#T2 -->
...
form on or alongside the primary content, which keeps playing — except
where the selected option's `@layout` is `linear`, the full-screen
takeover of last resort, which replaces the primary content for the
duration of the option and resumes it where it stopped (§5.3.7.3).
...
<!-- refine: v8-dash-conformance-audit.md#M2 -->
The child is a **core-namespace** element inside a foreign-namespace
parent. `<ImportedMPD>` is declared only within `PeriodType`, so at
this position it carries no schema declaration at all. The placement is
permitted by the base specification's own removal rule — *"The MPD
...
schema and that conforms to this document"* (DASH §5.2.1) — which this
construct satisfies: removing it leaves the SGAI subtree valid, and
removing the SGAI subtree by the next paragraph of the same clause
takes it along. The reference processing model of DASH §5.3.2.6.3 does
not apply, because it combines the imported MPD into a Linked Period
and there is no Period here; this specification supplies the resolution
behaviour itself (§4.6). The nesting is deliberate: it is what makes
the creative opaque to a Player that predates this specification
(§4.7.5), which is the behaviour this construct wants, since such a
Player has no slot to play it in. `[inferred]` — a schema whose target
namespace is `urn:svta:dash:sgai:2026` cannot declare an element in the
DASH namespace, so an SGAI schema admits this child only through a
namespace wildcard, and under `lax` processing that child is not
validated against `ImportedMpdType`.
...
<!-- refine: v8-spec-validation.md#T1 (R27.1) -->
An L-shape presentation option MUST carry exactly **one** ad creative
— a single URL carrying an image, a video or an HTML creative — and
that creative MUST be placed full-frame in the background. The shrunk
primary content is composited
...
<!-- refine: v8-spec-validation.md#T1 (R26.1) -->
The background element is a still **image** — a branding surface, never a
video and never an HTML surface, so it never consumes a video decoder — and
it is the **advertiser's** creative, mirroring the IAB "Double Box Video +
Background" model. It MUST be carried as a composition attribute of the
layout rather than as one of the candidate's alternative presentation
options: the Player does not walk it the way it walks the options, it
composites it as part of rendering the layout once that layout is chosen.
...
<!-- refine: v8-spec-validation.md#T1 (R3.2, R26.3, R27.3) -->
...
Player MUST evaluate in §4.6.5, and what an APS holding capability
parameters evaluates in §5.8.2. Every layout and form pair has a row,
so a Player of any device class has a defined outcome — render, fall
back, or skip — for every opportunity type this specification defines,
and MUST select only an option whose row admits its class.
...
<!-- refine: v8-dash-conformance-audit.md#M3 -->
| Ad | Position of the tracking `<EventStream>` |
|---|---|
| Linear | Inside the sub-MPD's `<Period>`, one per ad |
| Non-linear, `video` form | Inside the form's sub-MPD `<Period>`, **or** directly inside the `<svta:Candidate>`. The two positions are equivalent for presentation; they differ in whether schema validation reaches the carrier (see the note below). |
...
> specification places `<EventStream>` as a child of `<Period>`, and
> declares the element only within `PeriodType`. Carrying it directly
> inside `<svta:Candidate>` is **not** foreign-namespace open content —
> `<EventStream>` is in the DASH namespace, which every `##other`
> wildcard excludes by definition. It is admissible under the base
> specification's removal rule instead: *"The MPD shall be authored
> such that, after XML attributes or elements in the DASH namespace but
> not in the XML schema documented in Annex B are removed, the result
> is a valid XML document formatted according to that schema and that
> conforms to this document"* (DASH §5.2.1). One consequence follows.
> Because the element sits outside the schema at that position,
> `@schemeIdUri`'s required use, the typing of `@presentationTime` and
> `@id`, and the prohibition on `@messageData` are not enforced by
> schema validation there, while the identical carrier inside a sub-MPD
> `<Period>` is fully validated; a Player therefore validates the
> callback attributes itself in the second position. The placement is
> also novel relative to the canonical one, so a validator or an
...
<!-- refine: v8-spec-validation.md#T1 (R28#p1) -->
...
The following items from the v8 analyses could not be resolved within a
minor refinement (they require requirement-level changes, `context/`
edits, or decisions the specification's owner takes). They are carried
forward for the next major build to address.

| # | Source | Issue ID | Summary | Why minor refinement is insufficient |
|---|--------|----------|---------|--------------------------------------|
| 1 | validation | F1 (EC-1) | `@executeOnce="true"` and `repeat` are jointly unsatisfiable on a pause window. | Ranking them decides whether a Publisher or APS declaration stops meaning what it says; `context/` ranks neither. Reserved for the owner's review pass. |
| 2 | validation | F2 (EC-2) | What `@executeOnce` bounds on an overlay window carrying several candidates. | `context/` defines the attribute for the pause family only, so either reading adds a requirement. |
| 3 | validation | F3 (EC-3) | The de-duplication key is not well defined across the two carrier placements one candidate may use. | The remedy is a normalisation rule, which changes what a conformant Player fires. |
| 4 | validation | F4 (EC-5, A-6) | Whether a non-linear presentation may outlive the `<Event>@duration` that declared its window. | The cap-versus-window question is requirement-level and `context/` conflates the two bounds. |
| 5 | validation | F5 (EC-6) | A hybrid break whose linear portion declares `@clip="false"`, against the concurrent overlay's own window. | Either resolution extends a window the Publisher declared or drops a composition the use cases describe. |
| 6 | validation | F6 (G-6) | What a zero `@maxDuration` means on a pause-trigger window. | Both readings change whether a resolution request is issued at all, which the APS observes. |
| 7 | validation | F7 (G-4) | The tracking-carrier narrowing drops R13.1's alternative without documenting the omission. | It depends on resolving A-5 in `context/` first; applying it here would settle a requirement-level disagreement. |
| 8 | validation | F8 (R1.3, A-3) | Whether the two attribute-use narrowings override base semantics. | R1.3 and R4.10 disagree; reconciling them is a requirements change. |
| 9 | validation | C1 (A-1) | R4.2 and R31.2 contradict each other about which families the cap bounds by cumulative duration. | The edit is to `context/03-requirements.md`, which a minor refinement does not touch. |
| 10 | validation | C2 (A-5, G-4) | R6.2, R13.1 and R13.4 state three different forces for one tracking-carrier rule. | Same: a `context/` edit, and G-4 cannot be closed either way until it lands. |
| 11 | validation | C3 (A-2, G-7) | UC-03 names layouts outside the closed set and asks for a per-slot concurrency cap. | The edit is to `context/04-use-cases.md`. |
| 12 | validation | C4 (A-4) | R26.1's "composition attribute of the slot / layout" is read two ways. | A `context/` edit; the specification already states the layout reading. |
| 13 | validation | C5 (A-3) | R1.3 as written forbids what R4.10 mandates. | A `context/` edit; the specification carries its own divergence note meanwhile. |
| 14 | validation | C6 (A-6, EC-5) | R14.2 names the opportunity window where it means the cap. | A `context/` edit, and it is the source of F4. |
| 15 | validation | C7 (EC-4) | R25.1 promises a freeze no Player can deliver past the time-shift buffer. | A `context/` edit; §4.6.17 already carries the bound the requirement lacks. |
| 16 | validation | C8 (A-7) | 23 of 123 criteria in `context/` carry no RFC 2119 modal. | The output cannot state a force its source does not state; six of them are plainly obligations and need the modal added upstream. |
| 17 | audit | M4 | DASH §8.14 extends DASH §8.12 without naming which of its two CMAF profiles, so a `ListMPD`'s inherited constraint set is undetermined. | Only the base specification can settle it; it is a working-group question, not a wording fix. |
| 18 | audit | M5 | The empty linear resolution document against the profile constraints it inherits. | Unresolvable while M4 is open, since the inherited set is unidentified. |
| 19 | detail review | flag 2 | The XML examples share no prologue convention: the declaration appears on 33 of 49 full-MPD examples, `xsi:schemaLocation` on 14, `@publishTime` on 10 of 15 primary manifests. | Which convention wins is an authorial choice and applying it touches up to 35 code blocks for no change in meaning; the log recommends normalising on a build rather than by hand. |
| 20 | detail review | flag 1 (b) | `context/06`'s scheme-URI pattern does not show the category segment the four minted URIs carry. | The specification side is applied (§2.1 now states the reading); amending the pattern is a `context/` edit. |
