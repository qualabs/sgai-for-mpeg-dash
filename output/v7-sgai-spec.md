...
Linear SGAI already exists in ISO/IEC 23009-1:2025 — the
...
- **`@mediaType` versus `@mimeType`.** Two attributes with
  non-overlapping domains. `@mediaType`, on
  `<svta:RenderableAsset>` (§5.3.1), is this specification's
  creative-carrier enum, whose admissible values are `video`, `image`
  and `html`. `@mimeType`, on `<AdaptationSet>` and `<Representation>`
  inside a sub-MPD, is the baseline DASH IANA media type, bound by RFC
  4337 to `video/mp4`, `audio/mp4` and `application/mp4`. This
  specification keeps them strictly separate.
...
three. They are the value space of `@mediaType` on a presentation
option (§5.3.1).

| `@mediaType` | Carrier | Concrete media types |
|---|---|---|
...
the script inside an HTML document and uses `@mediaType="html"`.
...
specification semantics are altered, and that every new construct
...
| *Which* slot family, and what constraints bind the slot — allowed layouts, duration cap, concurrency cap | Publisher |
...
when the resolution document declares more than one form for one slot,
the Player presents them one after another, in the order the forms
appear in the document, each starting when the previous one ends —
...
|---|---|---|
| Foreign-namespace open content | §5.2.1 | Every XML element and attribute in `urn:svta:dash:sgai:2026` (§5.1.3, §5.1.4, §5.2.2, §5.3, §5.6, §5.7) |
| Application-level Event Streams | §5.10 | The overlay and pause-trigger opportunity declarations, and the tracking carrier (§5.1, §5.5) |
| Vendor descriptor schemes | §5.8.4.8, §5.8.4.9 | Admissible as an alternative carrier; not used by this edition, for the reason recorded in §5.3.2 |
...
An Annex F construction — a new delivery format with a new
Interoperability Point URI in `MPD@profiles` — is admissible only when
...
chosen: at least one `<AdaptationSet>` is present in each `<Period>`
unless the Period's `@duration` is zero. An "events-only Period" of
non-zero duration is therefore not a legal carrier.
...
|---|---|---|---|---|---|
| `urn:svta:dash:event:sgai-overlay:2026` EventStream | `<Period>` child in the main MPD | §5.10 per-scheme skip | Skips the EventStream and every Event it carries; primary content continues | Removing it leaves a valid MPD | Event Stream (§5.10) |
| `urn:svta:dash:event:sgai-pause-trigger:2026` EventStream | `<Period>` child in the main MPD | §5.10 per-scheme skip | Skips it; a viewer pause produces no ad and no side effect | Removing it leaves a valid MPD | Event Stream (§5.10) |
| `<svta:OverlayPresentation>` | `<Event>` child | §5.2.1 foreign namespace | Discarded with its whole subtree, together with the skipped Event | The Event carries no baseline child that a legacy Player needs | Foreign-namespace open content |
| `<svta:PauseAdPresentation>` | `<Event>` child | §5.2.1 foreign namespace | Discarded with its whole subtree | Same | Foreign-namespace open content |
| `<svta:OverlayList>` | `<Period>` child in the resolution document | §5.2.1 foreign namespace | Never reached: the slot that resolves to this document is itself skipped at the main-MPD level | The enclosing Period is valid without it | Foreign-namespace open content |
| `<svta:Candidate>` | `<svta:OverlayList>` child | §5.2.1 foreign namespace | Discarded with the parent subtree | n/a — never reached | Foreign-namespace open content |
| `<svta:RenderableAsset>` | `<svta:Candidate>` child | §5.2.1 foreign namespace | Discarded with the parent subtree | n/a — never reached | Foreign-namespace open content |
| `<svta:BackgroundElement>` | `<svta:RenderableAsset>` child | §5.2.1 foreign namespace | Discarded with the parent subtree | n/a — never reached | Foreign-namespace open content |
| `<svta:Click>` | `<svta:Candidate>` child | §5.2.1 foreign namespace | Discarded with the parent subtree; the click is inert on a legacy Player | n/a — never reached | Foreign-namespace open content |
| `<svta:AdSystem>`, `<svta:AdTitle>`, `<svta:Advertiser>`, `<svta:UniversalAdId>` | `<svta:Candidate>` child | §5.2.1 foreign namespace | Discarded with the parent subtree; nothing in the ad presentation depends on them | n/a — never reached | Foreign-namespace open content |
| Reserved capability parameters | Query string on the resolution request | Not an MPD construct | An APS that does not implement them answers without them | n/a | HTTP query parameter (§5.8) |

The legacy-Player walk-through is the same in every row of the table
and is stated once here. A legacy Player parsing a main MPD authored
per this specification meets the SGAI `<EventStream>` first. It does
not implement the scheme URI, so it skips the EventStream and every
`<Event>` inside it, and the foreign-namespace child element inside
each Event is discarded with it. The remaining document is exactly the
baseline MPD — the primary content's `<Period>`, its
`<AdaptationSet>`s, and whatever standard linear break the Publisher
authored as the VOD fallback — which parses and plays. Nothing the
legacy Player needs was nested inside an SGAI element: every baseline
construct is a sibling of the SGAI constructs, never a child. Because
the legacy Player never recognises the opportunity, it never issues
the resolution request, so it never reaches a resolution document
either. It logs no error above the informational level, renders no
artefact, and fires no beacon for the skipped opportunity.
...
| `@earliestResolutionTimeOffset` | no | `xs:unsignedLong` | 60 s when absent `[inferred]` | Offset subtracted from the event's `presentationTime` to obtain the Earliest Resolution Time, in `@timescale` units. |
...
> `[inferred]` The base specification's XML Schema declares
> `@earliestResolutionTimeOffset` as optional with **no schema
> default**; the 60-second value above is the semantic default
> carried in the clause's prose and reused by this specification for
> consistency with `<ImportedMPD>@earliestResolutionTimeOffset`,
> whose schema default is `60.0` seconds. An implementation that
> needs the value to be unambiguous declares it explicitly on the
> slot, which §4.3.5 asks for in any case.
...
Annex L's `urn:mpeg:dash:nonlinearplayback:2020`, despite its name,
models interactive storylines as a graph of Periods with viewer decision
points; it defines no overlay rendering and no ad semantics. A new
...
| `@earliestResolutionTimeOffset` | no | `xs:unsignedLong` | 60 s when absent `[inferred]` | Offset subtracted from the event's `presentationTime` to obtain the Earliest Resolution Time, in `@timescale` units. Same name, type and semantics as on the linear events. |
...
uses for `@profiles`, `@codecs` and `@dependencyId`. The element form
is warranted only when each item carries its own attributes or
children, which a layout token does not.
...
| `@maxDuration` | yes | `xs:unsignedLong` | — | Maximum display duration of the pause ad before automatic dismissal, in the parent `<EventStream>@timescale` units. |
| `@earliestResolutionTimeOffset` | no | `xs:unsignedLong` | 60 s when absent `[inferred]` | Offset subtracted from the event's `presentationTime` to obtain the Earliest Resolution Time. The Player MAY resolve speculatively at that time or lazily at the moment of pause (§8.5). |
...
origin-level robustness on a sub-MPD or segment fetch, not for
chaining two different opportunities. The MPD fallback scheme
`urn:mpeg:dash:fallback:2016` (§5.11.3) chains whole presentations on
an unrecoverable playout error; its ordering rule — the content author
expresses preference by order, the first having the highest preference
— is the same rule this specification applies, and it is cited here as
the edition's own precedent for document-order-as-preference, but its
    ...
    <svta:OverlayList>
      <svta:Candidate id="cand-1" duration="PT10S">
        ...
        <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
          ...
          <Event presentationTime="5000" id="b2">https://tracker.example.com/midpoint?ad=1</Event>
          <Event presentationTime="10000" id="b3">https://tracker.example.com/complete?ad=1</Event>
        </EventStream>
...
| Attribute | Required | Type | Default | Description |
|---|---|---|---|---|
| `@profiles` | yes | whitespace-separated list of `xs:anyURI` | — | Includes `urn:svta:dash:profile:sgai-overlay-list:2026`. |
| `@type` | yes | enum (`static` \| `dynamic`) | `static` | `static` on an Overlay Resolution Document: the document is a complete answer at the moment it is served. |
...
(§5.3.11) combines media content components across Adaptation Sets into
one experience intended for joint decoding and rendering — NGA track
mixing, base plus enhancement layer, stereoscopic video — so it orders
media variants of one presentation rather than alternative presentations
of one ad, and the client is not required to take the first in document
order. `@selectionPriority` (§5.3.7.2 of the base specification) is a
non-binding author hint on `RepresentationBaseType` where higher
integers express higher preference — the opposite direction from
document order, and applied after capability filtering rather than as
the walk itself. `urn:mpeg:dash:fallback:2016` (§5.11.3) states this
specification's ordering rule almost verbatim, but at the granularity of
whole MPD URLs and triggered by an unrecoverable playout error rather
than by a capability check. Reusing any of the three would put an
ad-level choice on a media-level construct, or invert the ordering
convention. The option element is therefore new, and it is kept minimal:
two attributes and, for a video form, one baseline child.

#### 5.3.1 Attributes

| Attribute | Required | Type | Default | Description |
|---|---|---|---|---|
| `@mediaType` | yes | enum (`video` \| `image` \| `html`) | — | The creative-carrier form. Value space in §5.3.2. |
...
| `@assetUrl` | conditional | `xs:anyURI` | — | The creative's URL when `@mediaType` is `image` or `html`. Absent when `@mediaType` is `video`, where the creative is reached through the `<ImportedMPD>` child (§5.3.4). |
...
When `@mediaType` is `video`, the creative is carried through an
...
capability parameters evaluates in §5.8.4.
...
| `pause-ad` | `video` | 1 (the primary content is paused and holds its frame; on a device that cannot re-task the decoder, 2) | none | D1, D2 |
...
`urn:mpeg:dash:event:callback:2015`, defined in §4.7 and §5.10.4.5 of
the base specification. This specification introduces **no** tracking
scheme of its own.
...
| `@value` | no | `xs:string` | — | Inherited from §5.10. Informational at the Player level: a Player does not key de-duplication on it. The value `1` is used in this document's examples for readability and alignment with existing callback deployments. |
...
| `@id` | yes | `xs:string` | — | Identifier of the beacon, used by the Player for de-duplication and by the implementation for logging. |
...
> as a child of `<Period>` or of `<MPD>`. Carrying it directly inside
> `<svta:Candidate>` is admissible as foreign-namespace open content
> but is novel relative to that canonical placement, so a validator or
> analytics pipeline that locates callback event streams by scanning
> only `<Period>` and `<MPD>` children misses the carrier attached to
> a non-video candidate. Tooling implementing this specification scans
> inside `<svta:Candidate>` as well.
...
The element is a child of `<svta:Candidate>`, so the ClickThrough
travels with the ad rather than with the slot: a Publisher configures
nothing beyond permitting the ad.
...
`<RequestParam>` element of type `ExtendedUrlInfoType`, carried by an
`<EssentialProperty>` or `<SupplementalProperty>` descriptor with
`@schemeIdUri="urn:mpeg:dash:urlparam:2025"`. The attribute that
scopes the template to a slot's resolution request is
`@includeInRequests`, whose admissible values are a whitespace-
separated list of request types; the value that names an
alternative-MPD resolution request is **`altmpd`**.
...
ad played. A non-linear slot never suspends the main timeline at all.
...
2. The first window cannot be accessed and the second answers with a
   document carrying no candidates. The fallback was used as declared,
   and the opportunity still resolved to no ads.
...
reading a value the document does declare.
...
- A callback `<EventStream>` carried directly inside
  `<svta:Candidate>` rather than under a `<Period>` or the `<MPD>`
  (§5.5.2).
...
<MPD xmlns="urn:mpeg:dash:schema:mpd:2011"
  ...
  <Period id="1" start="PT0S" duration="PT15S">
    ...
    <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
                 value="1" timescale="1000">
      <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=101</Event>
      <Event presentationTime="0"     id="b2">https://tracker.example.com/start?ad=101</Event>
      <Event presentationTime="3750"  id="b3">https://tracker.example.com/firstQuartile?ad=101</Event>
      <Event presentationTime="7500"  id="b4">https://tracker.example.com/midpoint?ad=101</Event>
      <Event presentationTime="11250" id="b5">https://tracker.example.com/thirdQuartile?ad=101</Event>
      <Event presentationTime="15000" id="b6">https://tracker.example.com/complete?ad=101</Event>
    </EventStream>
...
| Class | Player decision | What the viewer sees |
|---|---|---|
| D1 | Transitions from primary to ad and back. MAY pre-buffer ad 1 on the second decoder while the primary finishes its last frames. | Playback transitions to two full-screen ads, then back to the live content near the slot position. |
  ...
  <Period id="resolution" duration="PT0S">
    <svta:OverlayList>
      ...
      <svta:Candidate id="cand-301-a" duration="PT10S">
        ...
        <svta:RenderableAsset mediaType="video" layout="linear">
          <ImportedMPD earliestResolutionTimeOffset="5">https://adcdn.example.com/301/takeover.mpd</ImportedMPD>
        </svta:RenderableAsset>

        <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
                     value="1" timescale="1000">
          <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=301</Event>
          <Event presentationTime="5000"  id="b2">https://tracker.example.com/midpoint?ad=301</Event>
          <Event presentationTime="10000" id="b3">https://tracker.example.com/complete?ad=301</Event>
        </EventStream>
  ...
  <Period id="resolution" duration="PT0S">
    <svta:OverlayList>
      <svta:Candidate id="cand-402" duration="PT10S">
        <svta:RenderableAsset mediaType="video" layout="overlay-lower-third">
        ...
        <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
                     value="1" timescale="1000">
          <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=402</Event>
          <Event presentationTime="10000" id="b2">https://tracker.example.com/complete?ad=402</Event>
        </EventStream>
      </svta:Candidate>
    </svta:OverlayList>
  </Period>
  ...
  <Period id="resolution" duration="PT0S">
    <svta:OverlayList>
      <svta:Candidate id="cand-501" duration="PT30S">
        ...
        <svta:RenderableAsset mediaType="video" layout="pause-ad">
          <ImportedMPD>https://adcdn.example.com/501/pause.mpd</ImportedMPD>
        </svta:RenderableAsset>

        <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
                     value="1" timescale="1000">
          <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=501</Event>
          <Event presentationTime="15000" id="b2">https://tracker.example.com/midpoint?ad=501</Event>
          <Event presentationTime="30000" id="b3">https://tracker.example.com/complete?ad=501</Event>
        </EventStream>
...
| Class | Player decision | What the viewer sees |
|---|---|---|
| D1 | Plays each ad in order, switching the decoder source between them. MAY pre-buffer ad N+1 on the second decoder. | A break of consecutive ads, then the primary content. |
  ...
  <Period id="resolution" duration="PT0S">
    <svta:OverlayList>
      <svta:Candidate id="cand-901" duration="PT15S">
        ...
        <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
                     value="1" timescale="1000">
          <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=901</Event>
          <Event presentationTime="7500"  id="b2">https://tracker.example.com/midpoint?ad=901</Event>
          <Event presentationTime="15000" id="b3">https://tracker.example.com/complete?ad=901</Event>
        </EventStream>
...
<svta:Candidate xmlns:svta="urn:svta:dash:sgai:2026"
  ...
  <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
               value="1" timescale="1000">
    <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=1001</Event>
    <Event presentationTime="15000" id="b2">https://tracker.example.com/complete?ad=1001</Event>
  </EventStream>
</svta:Candidate>
```
...
<svta:Candidate xmlns:svta="urn:svta:dash:sgai:2026"
  ...
  <svta:RenderableAsset mediaType="video" layout="squeezeback-double-box">
    <ImportedMPD>https://adcdn.example.com/1001/ad.mpd</ImportedMPD>
  </svta:RenderableAsset>
  <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
               value="1" timescale="1000">
    <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=1001b</Event>
    <Event presentationTime="15000" id="b2">https://tracker.example.com/complete?ad=1001b</Event>
  </EventStream>
</svta:Candidate>
```
...
<svta:Candidate xmlns:svta="urn:svta:dash:sgai:2026"
  ...
  <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
               value="1" timescale="1000">
    <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=1101</Event>
    <Event presentationTime="7500"  id="b2">https://tracker.example.com/midpoint?ad=1101</Event>
    <Event presentationTime="15000" id="b3">https://tracker.example.com/complete?ad=1101</Event>
  </EventStream>
...
<svta:Candidate xmlns:svta="urn:svta:dash:sgai:2026"
  ...
  <EventStream schemeIdUri="urn:mpeg:dash:event:callback:2015"
               value="1" timescale="1000">
    <Event presentationTime="0"     id="b1">https://tracker.example.com/impression?ad=901</Event>
    <Event presentationTime="15000" id="b2">https://tracker.example.com/complete?ad=901</Event>
  </EventStream>
</svta:Candidate>
```
...
|---|---|
| §3 Terms and vocabulary | Every `@layout` and `@allowedLayouts` token an implementation emits or accepts is in §3.2; every `@mediaType` is in §3.3. |
...
| §7 Expected behaviour | The per-scenario tests T-P1..T-P13. |
...
- **An APS that can be made to fail deliberately** — timeout,
  non-`200`, truncated body, empty candidate list — for T-E1 to T-E5.
  The four failures must be independently triggerable, because the
  point of those tests is that the Player distinguishes them.
