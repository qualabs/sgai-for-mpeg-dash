...
**Status:** candidate specification, build iteration 11.1.
...
- **DR-6 — Four carriers exist for a non-AV asset**: (a) foreign-namespace
  ...
  <!-- refine: v11-dash-conformance-audit.md#E2 -->
  Both descriptors sit on `EventStream` and `Event` as well as on
  `AdaptationSet`, `Representation` and `SubRepresentation`; `Period` and
  `AlternativeMPDEventType` admit `SupplementalProperty` only.
...
3. **Legacy walk-through.** (i) A legacy Player that implements DASH Annex I.3
   ...
   <!-- refine: v11-dash-conformance-audit.md#C39 -->
   A legacy Player that does not implement DASH Annex I.3 does not recognise
   the MPD-level `EssentialProperty` of scheme `urn:mpeg:dash:urlparam:2025`
   that accompanies `RequestParam` (§5.8.1). *"If the scheme or the value for
   this descriptor is not recognized, the DASH Client is expected to ignore
   the parent element that contains the descriptor"* (DASH §5.8.4.8, NOTE 1);
   at MPD level that parent is the MPD, so such a Player does not play the
   primary content. The base qualifies its requirement for the descriptor:
   *"… unless the scheme is explicity allowed in a profile (e.g. ISO-BMFF
   Advanced Linear Profile defined in 8.13)"* (DASH Annex I.3.1). The cost is
   the base's, and the Publisher takes it on by attaching `RequestParam`,
   which is optional (§5.8.1).
...
| The `custom` rectangle's notation | The SRD convention: origin top-left, x to the right, y downward, width and height in a reference space; containment as *"the sum of object_x and object_width is smaller or equal to total_width"* | DASH Annex H.2.2 (axes); Table H.1, DASH Annex H.2.3 (containment) <!-- refine: v11-dash-conformance-audit.md#E4 --> |
...
- **A superseded linear event is not executed while its window presents an
  ...
  <!-- refine: v11-dash-conformance-audit.md#C16 -->
...
<!-- refine: v11-dash-conformance-audit.md#C42 -->
...
- Types are those of W3C XML Schema Part 2 unless defined here. Four types
  are defined here, under the names the schema of §5.10.1 declares:
  <!-- refine: v11-detail-review.md#flag-1 -->
...
| `@allowedLayouts` | O | `LayoutTokenListType` <!-- refine: v11-detail-review.md#flag-1 --> | the family default (§3.4.3) | The layouts the window admits, drawn from §3.4.2. |
| `@customRegion` | O | `PercentRectType` <!-- refine: v11-detail-review.md#flag-1 --> | the whole viewport | The region inside which a `custom` option's rectangle lies (§5.3.5). Present only when `@allowedLayouts` lists `custom`. |
...
| `@allowedLayouts` | O | `LayoutTokenListType` <!-- refine: v11-detail-review.md#flag-1 --> | the family default (§3.4.3) | The layouts the window admits. |
...
| `@layout` | M | `LayoutTokenType` <!-- refine: v11-detail-review.md#flag-1 --> | — | One token of §3.4.2 (§5.3.3). |
| `@rect` | CM | `PercentRectType` <!-- refine: v11-detail-review.md#flag-1 --> | — | The overlay's rectangle. Present if and only if `@layout="custom"` (§5.3.5). |
...
1. **SGAI event streams under the Advanced Linear profile.** Whether an
   ...
   and *"may not be presented"* (DASH §8.13.2.1).
   <!-- refine: v11-dash-conformance-audit.md#C43 -->
   The base's own Advanced Linear example (DASH Annex K.6.4) carries a Period
   ...
   non-exhaustive at least for its own schemes. Two points remain open:
   whether an application scheme counts as a conforming Period element, and
   whether an SGAI `RequestParam` fits DASH §8.13.2.4 and DASH §8.13.2.7, whose
   lists of `@includeInRequests` values contain no URN.
   <!-- refine: v11-dash-conformance-audit.md#E3 -->
   The annexes use the live profile. Needs input from the base
   specification's editors.
...
6. **A pause window inside an alternative presentation.** The `Metrics`
   ...
   <!-- refine: v11-dash-conformance-audit.md#E5 -->
   An alternative presentation reached through `@uri` is not SPS-bound: only
   ...
   it always starts at the live edge"*, DASH §5.16.2.2.6), which cannot be
   SPS. Its MPD may therefore carry a `Metrics` request of its own.
...
9. **The empty List MPD shape** (§5.2.3) rests on a zero-duration Period
   ...
   base anticipates *"merge process resulted in no available media"* (DASH
   §5.16.2.2.6). But the List profile *"is an extension of the ISO-BMFF CMAF
   Profile"* (DASH §8.14), and a Media Presentation conforms to a profile only
   if *"There is at least one Representation in each Period in the
   profile-specific MPD"* (DASH §8.1). The empty List MPD can therefore be a
   ...
   <!-- refine: v11-dash-conformance-audit.md#C26 -->
...
| R-SYN-5 | <!-- refine: v11-detail-review.md#flag-1 -->`PercentRectType` and `LayoutTokenListType` values with a comma separator, five numbers, a value above 100, an unknown token. | Rejected. |
...
The following items from the v11 analyses could not be resolved within a
minor refinement (they require requirement-level changes or architectural
decisions, or they are held for human review). They are carried forward for
the next major build to address.

| # | Source | Issue ID | Summary | Why minor refinement is insufficient |
|---|--------|----------|---------|--------------------------------------|
| 1 | validation | d-1 (G-1) | What the required cap of a pause window bounds | `context/` requires the cap and gives it no meaning; decided in R4.1, R4, R31.2, R4.11 |
| 2 | validation | d-2 (DP-1.2#p1, A-5) | `<svta:Ad>@duration` and the video sub-MPD's `Period@duration` declare one value twice | The only `contradicted` row; DP-1.2 has to admit the pair or say which is derived |
| 3 | validation | d-3 (G-2) | The nine questions of §8.13 are not recorded as open in `context/` | The owner decides or records them as deliberately open |
| 4 | validation | d-4 (R17.5, A-2) | R17.5 against R40.4 for a pause window with no relation | Requirement wording in R17.5 |
| 5 | validation | d-5 (R40.1, A-3) | Supersede on a pause window | Requirement wording in R40.1 |
| 6 | validation | d-6 (A-4) | Whether the rest of a window's span is presented after an alternative presentation ends | Two readings of R40.4 |
| 7 | validation | d-7 (G-3) | A standalone resolution document is not among R1.2's extension points | R1.2 and `08-dash-extension-rules.md` decide admissibility |
| 8 | validation | d-8 (A-14) | Which base skip declarations count under R35.8, and their order | Requirement wording in R35.8 |
| 9 | validation | d-9 (UC-08, A-7) | UC-08's reuse signal has no construct | Use-case or requirement change |
| 10 | validation | d-10 (UC-16, A-8) | UC-16 does not name the overlay form; the D2 cell disagrees with Annex P | Use-case change |
| 11 | validation | d-11 (A-9, A-6, A-1) | UC-12, R15.2 and R26.1 wording | Use-case and requirement wording; the spec already reads each one way |
| 12 | validation | d-12 (A-15) | Ten obligations of `context/` carry no RFC 2119 keyword | Requirement wording |
| 13 | validation | f-1 (EC-1) | Which timeline runs while a pause ad is on screen (dismissal, beacons, per-pass cap) | Flagged for human review (§5.b); two resolutions considered |
| 14 | validation | f-2 (A-12, L156); audit E1 | `(DASH §4.8.9)` in §1.3 cites a clause the base does not have (E1 confirms against the primary copy) | Flagged for human review (§5.b); the audit offers two fixes: drop the parenthesis, or cite Table 63 |
| 15 | validation | f-3 (A-13) | `overlay-lower-third` "across the bottom 30% of the frame" restates an IAB bound with no source | Flagged for human review (§5.b); drop the figure or cite the IAB section |
| 16 | audit | C19 | PLY-41 treats a List MPD with unrenderable candidates as not a failed execution; the base's open failure list covers missing media | Both remedies are requirement-level: restricting PLY-41 to the non-linear families contradicts R20.1, which applies to every family; recording a departure in §4.8.3 contradicts R20.1's statement that this is not an extension of the base condition, and a recorded exception to R1.5 is made at requirement level (as R40.8) |
| 17 | audit | C20 | Dropping List MPD Periods before play (PLY-34, PLY-35) is not recorded in §4.8.3 | Confining PLY-34/35 to `svta:OverlayList` contradicts R7.2 and R7.3; recording a new exception to R1.5 is made at requirement level (as R40.8) |
