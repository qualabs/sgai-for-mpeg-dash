...
The norm extends MPEG-DASH 6th edition (ISO/IEC 23009-1:2025):
...
The norm explicitly **does not** cover:
...
conforming to this norm MUST comply with the cited editions or
...
- **IAB Tech Lab — Ad Format Guidelines for Digital Video and CTV**,
  ...
  chapter 3 of this norm are sourced from this document and are
...
- **IAB Tech Lab Video Ad Serving Template (VAST) 4.x** — Used
  illustratively in annex A only. **This norm has no normative
...
This norm enumerates five device classes that capture the rendering
...
The accepted ad-type values for this edition of the norm are sourced
from the IAB document cited in chapter 2. The norm itself does not
...
A conformant implementation of this norm MUST satisfy all the
...
against this norm (in practice, the spec author).
...
- **B-01**: Declare every ad slot in the primary MPD as an `<Event>`
  ...
  the year-pinned URIs introduced by this norm (see Chapter 5 §5.2)
...
- **B-02**: On every ad slot (linear or non-linear), declare the
  ...
  `@maxDuration` on a slot defined by this norm is non-conforming.
...
- **P-01**: For every ad opportunity type defined in this norm,
...
- **P-09**: A legacy Player that does not implement an SGAI
  construct introduced by this norm MUST ignore the unknown
...
### 4.4 Cross-cutting obligations (norm-document level)

- **D-01**: This norm MUST NOT alter or override the semantics of
  any pre-existing MPEG-DASH 6th edition construct.
- **D-02**: This norm MUST NOT introduce a new tracking event
...
norm. The linear baseline constructs (`InsertPresentation`,
...
edition and are described here only insofar as the SGAI norm
...
The norm introduces three event scheme URIs under the SVTA Ads WG
...
These elements are new in this norm and live in the `svta:` namespace
declared in §5.2.
...
`urn:svta:dash:sgai-list:2026` (this norm's candidate-set profile).
...
This norm does NOT introduce a new tracking scheme. Both linear and
...
norm does not introduce one. Implementations that need to carry such
...
vendor URN. The norm does not pin the vendor namespace because
...
norm encounters them as unknown elements (`<svta:OverlayPresentation>`,
...
norm because A-02 requires every candidate to carry at least one
...
exposing the viewer to extraneous tracking. This norm does not
...
- Place vendor elements as children of `<ImportedMPD>` or as
  ...
  it describes.
- Document the vendor schema separately from this norm; the SGAI
  norm imposes no shape on vendor extensions beyond the
...
streams). This norm does not specify the API shape; common patterns
include:
...
the norm output's annex policy.
...
This concludes the norm document.
