...
| `allowedLayouts` | Yes | xs:string | `pause-ad` | Space-separated list of layout tokens. For pause-ad slots, `pause-ad` is the canonical token; `overlay-*` tokens MAY also be admissible when the Broadcaster wants to permit the same creative templates as a coexisting overlay. At least one token MUST be present. |
...
The following items from the v4 analyses could not be resolved
within a minor refinement (they require requirement-level
changes or architectural decisions). They are carried forward
