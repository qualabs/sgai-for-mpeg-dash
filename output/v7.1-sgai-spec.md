...
> - Minor refinement v7 -> v7.1: issues raised by
>   `v7-spec-validation.md`, `v7-detail-review.md` and
>   `v7-dash-conformance-audit.md`, applied without changing
...
§8.15, §7.3); **DR-2** foreign-namespace open content (§5.2.1);
**DR-3** the whole-subtree discard of an unimplemented foreign element
(§5.2.1 NOTE 2); **DR-4** the Annex F path to a non-ISO-BMFF delivery
...
Every clause cited in a DR-N label is a clause of the base
specification.
...
| `<svta:Click>` | `<svta:Candidate>` child | §5.2.1 foreign namespace (DR-2, DR-3) | Discarded with the parent subtree; the click is inert on a legacy Player | n/a — never reached | Foreign-namespace open content — DR-6(a) |
| `<svta:Click>` | `<Period>` child in a `ListMPD` | §5.2.1 foreign namespace (DR-2, DR-3) | Discarded with its subtree; the click is inert | The Period is valid without it | Foreign-namespace open content — DR-6(a) | <!-- refine: v7-spec-validation.md#T4 -->
| `<svta:AdSystem>`, `<svta:AdTitle>`, `<svta:Advertiser>`, `<svta:UniversalAdId>` | `<svta:Candidate>` child | §5.2.1 foreign namespace (DR-2, DR-3) | Discarded with the parent subtree; nothing in the ad presentation depends on them | n/a — never reached | Foreign-namespace open content — DR-6(a) |
...
The following items from the v7 analyses could not be resolved within a
minor refinement (they require requirement-level changes or
...
|---|--------|----------|---------|--------------------------------------|
| 1 | validation | F1 (A-1) | Several `<svta:Candidate>` in one document are a sequence in §4.6.7 and a set of alternatives in §4.6.6; what the Player does after one renders successfully is unstated | The validator routed it to §5.b (flagged for human review); either answer changes what an existing conformant document means, and the durable fix is R14 in `context/` (D3) |
| 2 | validation | F2 (A-4) | Splitting IAB *Double Box Video* into `squeezeback-double-box` and `squeezeback-double-box-with-background` makes an advertiser-supplied background Publisher-gated, which R26.2 does not contemplate | Routed to §5.b. Collapsing the tokens or keeping them is a layout-vocabulary decision against R12.2 / R26.2, not a wording fix |
| 3 | validation | F3 (A-6) | §5.1.1 marks `@maxDuration` required under this specification and carries the base schema's `2251799813685247` default in the same row | Routed to §5.b. Striking the default and downgrading the obligation have different consequences for a conformance checker |
| 4 | validation | F4 (G-2) | No Player behaviour for a slot authored without `@maxDuration` | Routed to §5.b. The two defensible answers — inherit the unbounded default, or decline the slot — are opposite outcomes and `context/` decides neither |
| 5 | validation | F5 (G-4) | The hybrid-break decision on a single-decoder device is stated only in the non-normative §8.4 and in an informative annex, while R3.2 requires defined per-class behaviour | Routed to §5.b. Promoting it fixes its strength (MUST vs SHOULD), which is a normative decision |
| 6 | validation | F6 (G-5) | The linear no-fill document trips the base specification's "List MPD, merge resulted in no available media" failure condition, which §4.6.8's divergence note does not cover | Routed to §5.b. Widening the divergence or re-encoding the linear no-fill both change an architectural choice |
| 7 | validation | F7 (EC-5) | `@allowedLayouts` is not bound to the slot family, so `pause-ad` passes the Publisher check on an overlay slot | Routed to §5.b. The positive form needs a family column in §3.2 and a new obligation in §4.3.3 |
| 8 | validation | F8 (EC-3) | The slot cap (`@timescale` units) and `<svta:Candidate>@duration` (`xs:duration`) are compared with no stated conversion or rounding rule | Routed to §5.b. Fixing rounding wrongly is worse than leaving it open, and where to state it is a real choice |
| 9 | validation | F9 (EC-4) | A `200` resolution document of the wrong family for the slot inherits no behaviour from E3 or E5 | Routed to §5.b. Deciding it without deciding E3's deliberately open case splits the treatment |
| 10 | validation | F10 (EC-2) | Whether the slot cap accrues while a non-linear form is paused is unstated | Routed to §5.b. Both readings are defensible and the decision also touches EC-1 |
| 11 | validation | D1 (G-7) | R1.3 forbids the narrowing §4.6.8 declares, so R1.3 and R20.1 + R30 cannot both hold | Deferred to `context/03-requirements.md`, which is read-only in a minor refinement. §1 and §4.1 now declare the exception (T5), but the requirement conflict stands |
| 12 | validation | D2 (G-8) | R25.1 promises a pause freeze "for the full duration of the pause" that the base specification's resumption-trimming rules do not permit | Deferred to `context/03-requirements.md`. The spec (§4.6.9) is the correct half |
| 13 | validation | D3 (A-1 / R14) | R14 does not map "sequence" and "alternatives" onto the resolution document | Deferred to `context/03-requirements.md`. It is the level at which F1 resolves |
| 14 | validation | D4 (G-3) | R10.3 requires a Positioning Templates section that no build has ever produced, and an `image` form has no HTML/CSS surface to carry position | Deferred to `context/03-requirements.md` and `context/02-actors.md` |
| 15 | validation | D5 (EC-1, G-6) | No use case covers a pause inside a pause-trigger window while a linear ad is on screen; UC-08 names a signal for an overlay candidate doubling as the pause-ad candidate that no construct expresses | Deferred to `context/04-use-cases.md`. Expressing the signal would add a construct |
| 16 | detail-review | flag-2 | §2.1 publishes `urn:svta:dash:event:sgai-overlay:2026`, inserting an `event:` segment the `context/06` pattern does not show | The reviewer's recommendation is to amend `context/06-naming-and-namespaces.md`, which is read-only here. The `event:` / `profile:` segment does real work and the spec is the correct half |
| 17 | detail-review | flag-3 | "Overlay" names a layout token, a slot family and a document class at once; the reviewer recommends renaming the document, the profile URI and `<svta:OverlayList>` to family-neutral terms | Renaming a registered profile URI and a published element is a wire-format change, not a wording fix. §5.2.2 now carries the note that disambiguates the overload; the rename stays open |
| 18 | audit | NC5 | §4.6.8's fall-through narrowing changes the base specification's §5.16.2.2.5 execution semantics for the linear family | Same finding as F6 / D1. Every remedy the audit offers either scopes the narrowing out of the linear family (a behaviour change) or drops a document-level conformance claim |
| 19 | audit | M1 | A callback `<EventStream>` directly inside `<svta:Candidate>` is unvalidatable and redefines the scheme's timebase | The audit's clarification is a new `<svta:TrackingEvents>` / `<svta:Beacon>` carrier — a new construct |
| 20 | audit | M2 | Core-namespace `<ImportedMPD>` inside `<svta:RenderableAsset>` has no global element declaration and carries a merge model that does not apply in that parent | The audit's clarification replaces it with `<svta:ImportedMPD>` or an `@mpdUrl` attribute — a new construct |
| 21 | audit | M4 | A `ListMPD` declaring only the MPEG list profile has its `<svta:Click>` removed by the §8.1 profile-conformance procedure | §5.6 now states the consequence. Requiring an SGAI profile URI on a `ListMPD` needs a linear profile URI that §2.1 does not mint, and the main-MPD half is an open decision |
| 22 | audit | M5 | `altmpd` is bound by Table I.4 to §5.16 alternative-MPD requests; a non-linear resolution request is not one | The audit's remedy mints `urn:svta:dash:request:sgai-resolution:2026` — a new URI and a new registry entry in §2.1 |
