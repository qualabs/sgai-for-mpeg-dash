...
- **ISO/IEC 23009-1:2025(E)** — *Information technology — Dynamic
  ...
  Annex I.4 (URL parameterisation `urn:mpeg:dash:schema:urlparam:2025`).
...
| `urn:svta:dash:sgai-layout:2026` | XML namespace / scheme-fragment | The layout-vocabulary anchor for chapter 3 §3.2 enum tokens (`linear`, `overlay`, etc.). |
...
- **Sub-MPD.** An MPD reached via `<ImportedMPD>` from a parent
  ...
  candidate's `<ImportedMPD>` child for non-linear). Bound by
  §5.3.2.6 to the SPS profile (§8.15) and therefore to
...
- **OOH** — Out-of-Home advertising (referenced in §3.2 for
  `in-scene`).
...
| `allowedLayouts` | Yes | xs:string | — | Whitespace-separated list of layout tokens drawn from the chapter 3 §3.2 enumeration. Each token MUST be one of the IAB-defined ad-type identifiers (or a documented sub-token). At least one token MUST be present. |
...
| `allowedLayouts` | Yes | xs:string | `pause-ad` | Whitespace-separated list of layout tokens. For pause-ad slots, `pause-ad` is the canonical token; `overlay-*` tokens MAY also be admissible when the Broadcaster wants to permit the same creative templates as a coexisting overlay. At least one token MUST be present. |
| `triggerMode` | No | xs:string | `manual` | Trigger origin. Admissible values: `manual` (viewer-initiated pause), `device-initiated` (screensaver / inactivity). For this edition only `manual` is normatively defined; `device-initiated` is reserved (see §1.1 OOS). |
...
| `urn:svta:dash:event:sgai-pause-trigger:2026` (`<svta:PauseAdPresentation>`) | `pause-ad`, `overlay`, `overlay-corner`, `overlay-lower-third`, `squeezeback`, sub-tokens thereof |
...
private namespace under the conventions in §2.1 (e.g.
`urn:qualabs:sgai:2026`). Players MUST ignore unknown
namespaces safely; ADSs and Broadcasters MUST NOT rely on
...
6th edition, namespace `urn:mpeg:dash:schema:urlparam:2025`) is
reused unchanged. The Broadcaster MAY declare query parameters
to append to the resolution-request URL via the descriptor's
...
informatively in chapter 6 §6.6 of [the linear baseline
adapter mapping carried by the SGAI implementer guide];
conformant implementations MUST be VAST-version-agnostic and
MUST be able to operate against an ADS that does not use VAST
at all.
...
all twenty test cases successfully has covered the principal
