...
Inputs: `../context/03-requirements.md` (mtime 2026-09-25 20:33:52 -0300) and
`../context/04-use-cases.md` (mtime 2026-09-25 19:55:18 -0300). 16 use cases
...
the coverage count. Governance = every conformance criterion binds the spec
document (R8, R9, R10, R18, R23 are labelled document-level; R11 binds the
spec's citations, its one Player criterion has no scenario to run in).
...
| R4 | A | A | A | A | A | A | · | A | A | A | · | · | A | A | A | A | 13 | (covered) |
...
|---|---|---|
| R24 | Non-AV asset carrier; image / HTML forms appear in UC-03, 05, 09, 10, 13–16, none states how the URL travels. | One line in UC-03 (Ad response): the image / HTML option URL rides a DR-6 carrier, not `@mimeType`. |
...
R32, R33 and R34 attach naturally to UC-05. R23 (previously listed as an
orphan) is tagged governance: its only criterion, R23.1, binds the spec
document, and R23 itself states it is "a requirement on the specification
...
|---|---|---|
| UC-07 | 1 | Only R1 is exercised. R1.4 (abort-and-continue) and R1.5 (base answer wins) are the plausible additions; R8 / R9 are the document-level side of the same contract. The thinness is inherent to a cross-cutting legacy scenario, not an under-constraint. |
...
(16 × 39 cells) and by per-R / per-UC count: **no mismatch**. Coherence issues
found in the inputs while building the matrix:

- **UC-10 vs R12.2.** UC-10 declares `squeezeback-double-box` and exercises
  an advertiser-supplied background; R12 enumerates that composition as
  `squeezeback-double-box-background` (*Double Box Video + Background*).
  UC-09 / UC-13 use the `-background` token for the same option.
- **UC-15 D2 and R30.** "The APS emits no candidate for this device" is the
  ...
  and does not name R30, so the step through R20.1 (no further window, primary
  continues) is implicit.
- **R10 is tagged governance but has one citation** (UC-03, "per R10"). The
  tag follows the prompt; the citation is real.
- **`04-use-cases.md` header** says the scenarios are grounded by "R1–R10";
  the file cites R1..R39.
